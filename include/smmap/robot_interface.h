...
class RobotInterface {
 public:
  ...
  RobotInterface(std::shared_ptr<ros::NodeHandle> nh, std::shared_ptr<ros::NodeHandle> ph);
  ~RobotInterface();
  ...
  std::pair<WorldState, std::vector<WorldState>> commandRobotMotion(const AllGrippersSinglePose& target_grippers_poses,
                                                                    const Eigen::VectorXd& target_robot_configuration,
                                                                    const bool robot_configuration_valid);

  bool testRobotMotion(const std::vector<AllGrippersSinglePose>& test_grippers_poses,
                       const std::vector<Eigen::VectorXd>& test_robot_configurations,
                       const bool robot_configuration_valid, const TestRobotMotionFeedbackCallback& feedback_callback);

  bool generateTransitionData(const std::vector<deformable_manipulation_msgs::TransitionTest>& tests,
  ...
  std::pair<WorldState, std::vector<WorldState>> testRobotMotionMicrosteps(
      ...
      const AllGrippersSinglePose& starting_grippers_poses, const AllGrippersSinglePose& target_grippers_poses,
  ...
  Eigen::VectorXd mapGripperMotionToRobotMotion(const AllGrippersSinglePoseDelta& grippers_delta) const;

  bool checkRobotCollision() const;

  std::vector<Eigen::VectorXd> getCloseIkSolutions(const AllGrippersSinglePose& target_poses,
                                                   const double max_gripper_distance) const;

  std::vector<Eigen::VectorXd> getCloseIkSolutions(const std::vector<std::string>& gripper_names,
                                                   const AllGrippersSinglePose& target_poses,
                                                   const double max_gripper_distance) const;

  std::pair<bool, Eigen::VectorXd> getGeneralIkSolution(const Eigen::VectorXd& starting_config,
  ...
  void setCallbackFunctions(
      ...
      const std::function<std::vector<Eigen::VectorXd>(const std::vector<std::string>& gripper_names,
                                                       const AllGrippersSinglePose& target_poses,
                                                       const double max_gripper_distance)>& close_ik_solutions_fn,
      const std::function<std::pair<bool, Eigen::VectorXd>(
          const Eigen::VectorXd& starting_config, const std::vector<std::string>& gripper_names,
  ...
  Eigen::Vector3d transformToFrame(const Eigen::Vector3d& point, const std::string& source_frame,
                                   const std::string& target_frame, const ros::Time& time = ros::Time(0)) const;
 ...
 private:
  ...
  actionlib::SimpleActionClient<deformable_manipulation_msgs::GenerateTransitionDataAction>
      generate_transition_data_client_;
 ...
 private:
  ...
  std::function<std::vector<Eigen::VectorXd>(const std::vector<std::string>& gripper_names,
                                             const AllGrippersSinglePose& target_poses,
                                             const double max_gripper_distance)>
      close_ik_solutions_fn_;
  std::function<std::pair<bool, Eigen::VectorXd>(const Eigen::VectorXd& starting_config,
                                                 const std::vector<std::string>& gripper_names,
                                                 const AllGrippersSinglePose& target_poses)>
      general_ik_solution_fn_;
  ...
  deformable_manipulation_msgs::ExecuteRobotMotionRequest toRosMovementRequest(
      const AllGrippersSinglePose& grippers_poses, const Eigen::VectorXd& robot_configuration,
  ...
  bool testRobotMotion_impl(const deformable_manipulation_msgs::TestRobotMotionGoal& goal,
                            const TestRobotMotionFeedbackCallback& feedback_callback);

  deformable_manipulation_msgs::TestRobotMotionGoal toRosTestPosesGoal(
      const std::vector<AllGrippersSinglePose>& grippers_poses,
      const std::vector<Eigen::VectorXd>& robot_configurations, const bool robot_configurations_valid) const;
  ...
  bool generateTransitionData_impl(const deformable_manipulation_msgs::GenerateTransitionDataGoal& goal,
                                   const GenerateTransitionDataFeedbackCallback& feedback_callback,
                                   const bool wait_for_feedback);
 ...
 public:
  deformable_manipulation_msgs::TransitionTest toRosTransitionTest(
      ...
      const AllGrippersSinglePose& starting_gripper_poses, const AllGrippersPoseTrajectory& path_to_start_of_test,
