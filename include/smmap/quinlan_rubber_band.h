...
class QuinlanRubberBand {
 public:
  ...
  static ObjectPointSet AggregateBandPoints(const std::vector<QuinlanRubberBand>& bands);
  static ObjectPointSet AggregateBandPoints(const std::vector<QuinlanRubberBand::Ptr>& bands);
  static ObjectPointSet AggregateBandPoints(const std::vector<QuinlanRubberBand::ConstPtr>& bands);

  static ObjectPointSet PointsFromBandPointsAndGripperTargets(const EigenHelpers::VectorVector3d& starting_points,
                                                              const PairGripperPositions& grippers_targets,
                                                              const size_t num_gripper_steps);
  static ObjectPointSet PointsFromBandAndGrippers(const QuinlanRubberBand& band,
  ...
  static QuinlanRubberBand::Ptr BandFromNodeTransformsAndGrippers(const EigenHelpers::VectorIsometry3d& node_transforms,
                                                                  const PairGripperPositions& grippers_position,
                                                                  const QuinlanRubberBand& template_band);

  static QuinlanRubberBand::Ptr BandFromWorldState(const WorldState& world_state,
                                                   const QuinlanRubberBand& template_band);

  QuinlanRubberBand(std::shared_ptr<ros::NodeHandle> nh, std::shared_ptr<ros::NodeHandle> ph, const Visualizer::Ptr vis,
                    const sdf_tools::SignedDistanceField::ConstPtr& sdf, const XYZGrid& work_space_grid,
  ...
  const EigenHelpers::VectorVector3d& forwardPropagate(const PairGripperPositions& gripper_positions, bool verbose);
  ...
  std::vector<Visualizer::NamespaceId> visualize(const std::string& marker_name, const std_msgs::ColorRGBA& safe_color,
                                                 const std_msgs::ColorRGBA& overstretched_color, const int32_t id,
                                                 const bool visualization_enabled = true) const;

  std::vector<Visualizer::NamespaceId> visualize(const EigenHelpers::VectorVector3d& test_band,
                                                 const std::string& marker_name, const std_msgs::ColorRGBA& safe_color,
                                                 const std_msgs::ColorRGBA& overstretched_color, const int32_t id,
                                                 const bool visualization_enabled = true) const;

  std::vector<Visualizer::NamespaceId> visualizeWithBubbles(const std::string& marker_name,
                                                            const std_msgs::ColorRGBA& safe_color,
                                                            const std_msgs::ColorRGBA& overstretched_color,
                                                            const int32_t id,
                                                            const bool visualization_enabled = true) const;

  std::vector<Visualizer::NamespaceId> visualizeWithBubbles(const EigenHelpers::VectorVector3d& test_band,
  ...
  static std::vector<Visualizer::NamespaceId> VisualizeBandSurface(const Visualizer::Ptr& vis,
                                                                   ...
                                                                   const std::string& ns, const int32_t id = 1);
  ...
  static std::pair<QuinlanRubberBand::Ptr, uint64_t> Deserialize(const std::vector<uint8_t>& buffer,
 ...
 private:
  ...
  bool sufficientOverlap(const double bubble_size_a, const double bubble_size_b, const double distance) const;
  ...
  [[gnu::warn_unused_result]] bool interpolateBetweenPoints(EigenHelpers::VectorVector3d& point_buffer,
