...
class TaskSpecification {
 public:
  ...
  static TaskSpecification::Ptr MakeTaskSpecification(std::shared_ptr<ros::NodeHandle> nh,
 ...
 public:
  ...
  TaskSpecification(std::shared_ptr<ros::NodeHandle> nh, std::shared_ptr<ros::NodeHandle> ph, Visualizer::Ptr vis,
  ...
  std::vector<Visualizer::NamespaceId> visualizeDeformableObject(const std::string& marker_name,
  ...
  double calculateError(const WorldState& world_state);
   ...
   */
  ObjectDeltaAndWeight calculateObjectErrorCorrectionDelta(const WorldState& world_state);

  bool taskDone(const WorldState& world_state);
  ...
  bool stretchingConstraintViolated(const ssize_t first_node_ind, const Eigen::Vector3d& first_node,
                                    const ssize_t second_node_ind, const Eigen::Vector3d& second_node) const;
   ...
   * @return
   */
  ObjectDeltaAndWeight calculateStretchingCorrectionDeltaFullyConnected(const ObjectPointSet& object_configuration,
   ...
   * @return
   */
  ObjectDeltaAndWeight calculateStretchingCorrectionDeltaPairwise(const ObjectPointSet& object_configuration,
                                                                  bool visualize) const;
   ...
   * @return
   */
  ObjectDeltaAndWeight calculateStretchingCorrectionDelta(const WorldState& world_state, bool visualize) const;
 ...
 protected:
  ...
  const double max_band_length_;  // Function of initial distance between the grippers, and max_stretch_factor_ - cached
                                  // value to allow for arbitrary starting setups of the environment from bullet/sim
 ...
 private:
  ...
  virtual double calculateError_impl(const WorldState& world_state) = 0;

  virtual ObjectDeltaAndWeight calculateObjectErrorCorrectionDelta_impl(const WorldState& world_state) = 0;

  virtual std::vector<ssize_t> getNodeNeighbours_impl(const ssize_t node) const = 0;

  virtual bool taskDone_impl(const WorldState& world_state) = 0;
};

/**
 * @brief The ModelAccuracyTestTask class. Essentially this is just a dummy placeholder for the task, and all output is
 * meaningless
 */
class ModelAccuracyTestTask : public TaskSpecification {
 public:
  ModelAccuracyTestTask(std::shared_ptr<ros::NodeHandle> nh, std::shared_ptr<ros::NodeHandle> ph, Visualizer::Ptr vis);

 private:
  virtual double calculateError_impl(const WorldState& world_state) override final;

  virtual ObjectDeltaAndWeight calculateObjectErrorCorrectionDelta_impl(const WorldState& world_state) override final;

  virtual std::vector<ssize_t> getNodeNeighbours_impl(const ssize_t node) const override final;

  virtual bool taskDone_impl(const WorldState& world_state) override final;
};

class CoverageTask : public TaskSpecification {
 public:
  CoverageTask(std::shared_ptr<ros::NodeHandle> nh, std::shared_ptr<ros::NodeHandle> ph, Visualizer::Ptr vis,
...
class DirectCoverageTask : public CoverageTask {
 public:
  DirectCoverageTask(std::shared_ptr<ros::NodeHandle> nh, std::shared_ptr<ros::NodeHandle> ph, Visualizer::Ptr vis);
 ...
 private:
  ...
  std::tuple<ssize_t, double, bool> findNearestObjectPoint(const WorldState& world_state,
...
class DijkstrasCoverageTask : public CoverageTask {
 public:
  ...
  struct Correspondences {
   public:
    ...
    std::vector<double>
        uncovered_target_points_distances_;  // Distance to the deformable object for each uncovered target point

    std::vector<std::vector<ssize_t>>
        correspondences_;  // Vector of size num_nodes_, each entry is a list of indices into the cover_points_ data
    std::vector<EigenHelpers::VectorVector3d>
        correspondences_next_step_;  // Vector of size num_nodes_, each entry is a list of "next steps" if moving
                                     // towards the corresponding target
    std::vector<std::vector<double>>
        correspondences_distances_;  // Vector of size num_nodes_, each entry is a list of distances to the
                                     // corresponding cover_point as listed in correspondences_
    std::vector<std::vector<bool>>
        correspondences_is_covered_;  // Vector of size num_nodes_, each entry is a list which desecribes if the
                                      // corresponding cover_point is already "covered"
  };

  DijkstrasCoverageTask(std::shared_ptr<ros::NodeHandle> nh, std::shared_ptr<ros::NodeHandle> ph, Visualizer::Ptr vis);

  ////////////////////////////////////////////////////////////////////////////////////////////////////////////
  // Virtual function wrappers
  ////////////////////////////////////////////////////////////////////////////////////////////////////////////

  const Correspondences& getCoverPointCorrespondences(const WorldState& world_state);

  ////////////////////////////////////////////////////////////////////////////////////////////////////////////
  // Interface functions used externally
  ////////////////////////////////////////////////////////////////////////////////////////////////////////////

  std::vector<EigenHelpers::VectorVector3d> findPathFromObjectToTarget(const WorldState& world_state,
  ...
  std::vector<double> averageDijkstrasDistanceBetweenGrippersAndClusters(const Eigen::Isometry3d& gripper_pose,
  ...
  void visualizeIndividualDijkstrasResult(const size_t cover_idx, const Eigen::Vector3d& querry_loc) const;
 ...
 private:
  ...
  virtual Correspondences getCoverPointCorrespondences_impl(const WorldState& world_state) const = 0;
  ...
  Eigen::Vector3d sumVectorFields(const std::vector<ssize_t>& cover_point_assignments,
                                  const Eigen::Vector3d& querry_loc) const;

  EigenHelpers::VectorVector3d followCoverPointAssignments(const Eigen::Vector3d& starting_pos,
 ...
 protected:
  ...
  /// Dijkstras results, indexed by goal index, then current node index - each entry is a (next_node, distance to goal)
  /// pair
...
class DistanceBasedCorrespondencesTask : public DijkstrasCoverageTask {
 public:
  DistanceBasedCorrespondencesTask(std::shared_ptr<ros::NodeHandle> nh, std::shared_ptr<ros::NodeHandle> ph,
                                   Visualizer::Ptr vis);

 private:
  virtual Correspondences getCoverPointCorrespondences_impl(const WorldState& world_state) const override final;

  std::tuple<ssize_t, double, ssize_t, bool> findNearestObjectPoint(const WorldState& world_state,
                                                                    const ssize_t cover_idx) const;
};

class FixedCorrespondencesTask : public DijkstrasCoverageTask {
 public:
  FixedCorrespondencesTask(std::shared_ptr<ros::NodeHandle> nh, std::shared_ptr<ros::NodeHandle> ph,
                           Visualizer::Ptr vis);
