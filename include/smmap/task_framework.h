...
class TaskFramework {
 public:
  ...
  TaskFramework(std::shared_ptr<ros::NodeHandle> nh, std::shared_ptr<ros::NodeHandle> ph, RobotInterface::Ptr robot,
                Visualizer::Ptr vis, TaskSpecification::Ptr task_specification);
 ...
 private:
  ...
  WorldState sendNextCommand(WorldState current_world_state);

  WorldState sendNextCommandUsingLocalController(const WorldState& current_world_state);

  WorldState sendNextCommandUsingGlobalPlannerResults(const WorldState& current_world_state);
  ...
  void visualizeProjectedPaths(const std::vector<EigenHelpers::VectorVector3d>& projected_paths,
                               const bool visualization_enabled);

  std::pair<std::vector<EigenHelpers::VectorVector3d>, std::vector<RubberBand>> projectFutureSystemState(
      const WorldState& current_world_state, const bool visualization_enabled);
  ...
  AllGrippersSinglePose getGripperTargets(const WorldState& world_state);

  void planGlobalGripperTrajectory(const WorldState& world_state);
  ...
  void updateModels(const WorldState& starting_world_state, const ObjectDeltaAndWeight& task_desired_motion,
                    const std::vector<DeformableController::OutputData>& suggested_commands, const ssize_t model_used,
                    const WorldState& world_feedback);

  Eigen::MatrixXd calculateProcessNoise(const std::vector<DeformableController::OutputData>& suggested_commands) const;
  ...
  void visualizeDesiredMotion(const WorldState& current_world_state, const ObjectDeltaAndWeight& desired_motion,
                              const bool visualization_enabled = true) const;

  void visualizeGripperMotion(const AllGrippersSinglePose& current_gripper_pose,
                              const AllGrippersSinglePoseDelta& gripper_motion, const ssize_t model_ind) const;

  void initializeBanditsLogging();

  void initializeControllerLogging();

  void logBanditsData(const WorldState& initial_world_state, const WorldState& resulting_world_state,
                      const std::vector<WorldState>& individual_model_results,
                      const Eigen::VectorXd& model_utility_mean, const Eigen::MatrixXd& model_utility_second_stat,
                      const ssize_t model_used);

  void controllerLogData(const WorldState& initial_world_state, const WorldState& resulting_world_state,
                         const std::vector<WorldState>& individual_model_results,
