...
class StretchingConstraintController : public DeformableController {
 public:
  StretchingConstraintController(std::shared_ptr<ros::NodeHandle> nh, std::shared_ptr<ros::NodeHandle> ph,
                                 RobotInterface::Ptr robot, Visualizer::Ptr vis,
 ...
 private:
  ...
  kinematics::Vector6d getConstraintAwareGripperDeltaSample(
      const Eigen::Isometry3d& gripper_pose, const CollisionData& collision_data, const double max_delta,
  ...
  void visualize_gripper_motion(const AllGrippersSinglePose& current_gripper_pose,
  ...
  double gripperCollisionCheckHelper(const AllGrippersSinglePose& current_gripper_pose,
                                     const AllGrippersSinglePoseDelta& test_gripper_motion) const;

  bool gripperCollisionCheckResult(const AllGrippersSinglePose& current_gripper_pose,
                                   const AllGrippersSinglePoseDelta& test_gripper_motion) const;
  ...
  bool stretchingDetection(const InputData& input_data, const AllGrippersSinglePoseDelta& test_gripper_motion);

  double stretchingFunctionEvaluation(const InputData& input_data,
                                      const AllGrippersSinglePoseDelta& test_gripper_motion);

  double ropeTwoGripperStretchingHelper(const InputData& input_data,
                                        const AllGrippersSinglePoseDelta& test_gripper_motion);

  bool ropeTwoGrippersStretchingDetection(const InputData& input_data,
                                          const AllGrippersSinglePoseDelta& test_gripper_motion);

  double clothTwoGripperStretchingHelper(const InputData& input_data,
                                         const AllGrippersSinglePoseDelta& test_gripper_motion);

  bool clothTwoGrippersStretchingDetection(const InputData& input_data,
                                           const AllGrippersSinglePoseDelta& test_gripper_motion);

  double evaluateStretchingConstraint(const std::pair<Eigen::Vector3d, Eigen::Vector3d>& stretching_constraint_data,
                                      const kinematics::Vector6d& gripper_delta) const;

  std::vector<std::pair<Eigen::Vector3d, Eigen::Vector3d>> stretchingCorrectionVectorsAndPoints(
      const InputData& input_data) const;
  std::vector<std::pair<Eigen::Vector3d, Eigen::Vector3d>> ropeTwoGrippersStretchingCorrectionVectorsAndPoints(
      const InputData& input_data) const;
  std::vector<std::pair<Eigen::Vector3d, Eigen::Vector3d>> clothTwoGrippersStretchingCorrectionVectorsAndPoints(
      const InputData& input_data) const;

  void visualizeCone(const Eigen::Vector3d& cone_direction, const double min_normalized_dot_product,
                     const Eigen::Isometry3d& pose, const int marker_id);
