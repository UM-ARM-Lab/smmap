...
class ConstraintJacobianModel final : public DeformableModel {
 public:
  ...
  ConstraintJacobianModel(std::shared_ptr<ros::NodeHandle> nh, const double translation_dir_deformability,
                          const double translation_dis_deformability, const double rotation_deformability,
 ...
 private:
  ...
  virtual void updateModel_impl(const WorldState& previous, const WorldState& next) final override;

  virtual ObjectPointSet getObjectDelta_impl(
      const WorldState& world_state, const AllGrippersSinglePoseDelta& grippers_pose_delta) const final override;
  ...
  double dirPropotionalModel(const Eigen::Vector3d node_to_gripper, const Eigen::Vector3d gripper_translation,
                             const double dist_rest) const;

  double disLinearModel(const double dist_to_gripper, const double dist_rest) const;
