...
class DeformableController {
 public:
  ...
  struct InputData {
   public:
    InputData(const WorldState& world_current_state, const DesiredDirection& desired_object_motion,
              const RobotInterface::Ptr& robot, const Eigen::MatrixXd& robot_jacobian, const bool robot_jacobian_valid,
              const std::vector<std::pair<CollisionData, Eigen::Matrix3Xd>>& poi_collision_data,
              const double max_step_size, const double max_robot_dof_step_size, const bool handle_overstretch)
  ...
  struct OutputData {
   public:
    ...
    OutputData(const size_t num_grippers, const ssize_t num_nodes_on_object, const ssize_t num_robot_dof)
    ...
    OutputData(const AllGrippersSinglePoseDelta grippers_motion, const ObjectPointSet object_motion,
               const Eigen::VectorXd robot_dof_motion)
        : grippers_motion_(grippers_motion), object_motion_(object_motion), robot_dof_motion_(robot_dof_motion) {}
  ...
  DeformableController(std::shared_ptr<ros::NodeHandle> nh, std::shared_ptr<ros::NodeHandle> ph,
                       RobotInterface::Ptr robot, Visualizer::Ptr vis, const DeformableModel::ConstPtr& model)
      : nh_(nh), ph_(ph), robot_(robot), vis_(vis), model_(model) {
    assert(model_ != nullptr);
  }

  OutputData getGripperMotion(const InputData& input_data) { return getGripperMotion_impl(input_data); }
 ...
 private:
  virtual OutputData getGripperMotion_impl(const InputData& input_data) = 0;
};
}  // namespace smmap
