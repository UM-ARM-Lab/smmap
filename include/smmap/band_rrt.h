...
class RRTNode {
 public:
  ...
  RRTNode(const RRTGrippersRepresentation& grippers_poses, const RRTRobotRepresentation& robot_configuration,
  ...
  static std::pair<RRTNode, uint64_t> Deserialize(const std::vector<uint8_t>& buffer, const uint64_t current,
...
class BandRRT {
 public:
  ...
  BandRRT(std::shared_ptr<ros::NodeHandle> nh, std::shared_ptr<ros::NodeHandle> ph, const WorldParams& world_params,
          const PlanningParams& planning_params, const SmoothingParams& smoothing_params, const TaskParams& task_params,
          const RubberBand::ConstPtr& template_band, Visualizer::Ptr vis, const bool visualization_enabled);

  //////// Planning functions //////////////////////////////////////////////////////////

  RRTPolicy plan(const RRTNode& start, const RRTGrippersRepresentation& grippers_goal_poses,
  ...
  static bool CheckTreeLinkage(const RRTTree& tree);

  static bool CheckPathLinkage(const RRTPath& path);

  static std::vector<Eigen::VectorXd> ConvertRRTPathToRobotPath(const RRTTree& path);
  ...
  std::vector<Visualizer::NamespaceId> visualizeTree(
      ...
      const std_msgs::ColorRGBA& color_b, const std_msgs::ColorRGBA& color_band, const bool draw_band = false) const;
  std::vector<Visualizer::NamespaceId> visualizeBothTrees() const;
  void deleteTreeVisualizations() const;
  std::vector<Visualizer::NamespaceId> visualizePath(const RRTPath& path, const std::string& ns_prefix,
                                                     const int32_t id, const bool draw_band = false) const;
 ...
 private:
  ...
  // Used for timing purposes
  // https://stackoverflow.com/questions/37786547/enforcing-statement-order-in-c
  int64_t nearestNeighbour(const bool use_forward_tree, const RRTNode& config);

  std::pair<int64_t, double> nearestNeighbourRobotSpace(const bool use_forward_tree, const RRTNode& config);

  int64_t nearestBestNeighbourFullSpace(const RRTNode& config);

  void rebuildNNIndex(std::shared_ptr<NNIndexType> index, std::vector<float>& nn_raw_data,
                      std::vector<size_t>& nn_data_idx_to_tree_idx, const RRTTree& tree, size_t& new_data_start_idx,
  ...
  size_t forwardPropogationFunction(RRTTree& tree_to_extend, const int64_t& nearest_neighbor_idx, const RRTNode& target,
                                    const bool allow_mistakes, const bool visualization_enabled_locally);
  ...
  void shortcutSmoothPolicy(RRTPolicy& policy, const bool visualization_enabled_locally);

  void shortcutSmoothPath(RRTPath& path, const bool maintain_goal_reach_invariant,
  ...
  std::pair<bool, RRTTree> forwardSimulateGrippersPath(const RRTTree& path, const size_t start_index,
