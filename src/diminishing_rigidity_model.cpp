...
DiminishingRigidityModel::DiminishingRigidityModel(std::shared_ptr<ros::NodeHandle> nh, const double deformability)
    : DiminishingRigidityModel(nh, deformability, deformability) {}

DiminishingRigidityModel::DiminishingRigidityModel(std::shared_ptr<ros::NodeHandle> nh,
...
void DiminishingRigidityModel::updateModel_impl(const WorldState& previous, const WorldState& next) {
...
Eigen::MatrixXd DiminishingRigidityModel::computeGrippersToDeformableObjectJacobian_impl(
  ...
  for (ssize_t gripper_ind = 0; gripper_ind < num_grippers; gripper_ind++) {
    ...
    for (ssize_t node_ind = 0; node_ind < num_nodes_; node_ind++) {
      const double dist_to_gripper = GetMinimumDistanceToGripper(grippers_data_[(size_t)gripper_ind].node_indices_,
                                                                 node_ind, object_initial_node_distance_);

      const Matrix3d& J_trans = gripper_rot;

      J.block<3, 3>(node_ind * 3, gripper_ind * 6) = std::exp(-translation_deformability_ * dist_to_gripper) * J_trans;
      ...
      const Vector3d gripper_to_node =
          current_configuration.col(node_ind) - grippers_pose[(size_t)gripper_ind].translation();
      ...
      J.block<3, 3>(node_ind * 3, gripper_ind * 6 + 3) = std::exp(-rotation_deformability_ * dist_to_gripper) * J_rot;
    }
  }
