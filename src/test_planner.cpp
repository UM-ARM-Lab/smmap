...
//    const AllGrippersSinglePoseDelta& selected_command = suggested_robot_commands[(size_t)model_to_use].first;
    ...
    **/
    ...
    #warning "This used to be getProjectedObjectDelta, but now there is only the one function."
    ObjectPointSet p_projected = model_list_[(size_t)model_to_use]->getObjectDelta(
                model_input_data, selected_command);
