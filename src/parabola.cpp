...
//    static pcl::ModelCoefficients::Ptr PlaneCoeffs(const CloudType::ConstPtr& cloud, const double inlier_threshold =
//    0.01)
//    {
...
//    static CloudType::Ptr ProjectToPlane(const CloudType::ConstPtr& cloud, const pcl::ModelCoefficients::ConstPtr&
//    coeffs)
//    {
...
static std::pair<double, double> FindParabolaCoeffs(const double x1, const double y1, const double length,
  ...
  const auto dIntegral = [](const double t) { return std::sqrt(1.0 + t * t); };
...
static sdf_tools::CollisionMapGrid ExtractSlice(const sdf_tools::SignedDistanceField& sdf,
                                                const Isometry3d& parabola_origin,
                                                const PairGripperPositions& gripper_positions, const double resolution,
                                                const double parabola_length, const Visualizer::Ptr vis) {
  ...
  if (second_point_parabola_frame.x() <= resolution) {
    ...
    const Vector3d offset(second_point_parabola_frame.x() / 2 - 1.5 * resolution,
                          std::max(0.0, second_point_parabola_frame.y()) - y_range - half_res, -half_res);

    sdf_tools::CollisionMapGrid grid(parabola_origin * Translation3d(offset), sdf.GetFrame(), resolution, x_cells,
                                     y_cells, z_cells, sdf_tools::COLLISION_CELL(1.0), sdf_tools::COLLISION_CELL(1.0));
  ...
  const auto coeffs =
      FindParabolaCoeffs(second_point_parabola_frame.x(), second_point_parabola_frame.y(), parabola_length);
  const double a = coeffs.first;
  const double b = coeffs.second;
  const auto parabola_eqn = [&](const double x) { return a * x * x + b * x; };
  ...
  const Vector3d grid_offset(x_min - 0.5 * (resolution * (double)x_cells -
                                            x_range),  // Center the valid region of the voxel grid between the grippers
                             y_min - 0.5 * (resolution * (double)y_cells -
                                            y_range),  // Center the valid region of the voxel grid on the parabola
                             -half_res);  // Shift half a cell to put the slice directly overtop of the grippers

  sdf_tools::CollisionMapGrid grid(parabola_origin * Translation3d(grid_offset), sdf.GetFrame(), resolution, x_cells,
                                   y_cells, z_cells, sdf_tools::COLLISION_CELL(1.0), sdf_tools::COLLISION_CELL(1.0));
  ...
  if (vis != nullptr) {
    ...
    vis->visualizeLineStrip(
        "parabola",
        parabola_origin * (ObjectPointSet(3, x.rows()) << x.transpose(), y.transpose(), z.transpose()).finished(),
    ...
    for (int64_t y_idx = 1; y_idx < y_cells - 1; ++y_idx) {
      ...
      // but the voxel center is on the wrong side of something
        ...
        const bool line_bisects_voxel = [&] {
          ...
          return (top_right_below_line ^ bottom_left_below_line) || (top_left_below_line ^ bottom_right_below_line);
        }();
        ...
        if ((line_bisects_voxel && above_parabola) || (parabola_bisects_voxel && below_line) ||
...
sdf_tools::CollisionMapGrid ExtractParabolaSliceBasic(const sdf_tools::SignedDistanceField& sdf,
  ...
  const Isometry3d parabola_origin = [&] {
    ...
    if (gripper_delta.head<2>().norm() <= resolution) {
      ...
      return Isometry3d((Matrix4d() << x_axis, y_axis, z_axis, gripper_positions.first, 0.0, 0.0, 0.0, 1.0).finished());
    }
...
sdf_tools::CollisionMapGrid ExtractParabolaSliceInPlane(const sdf_tools::SignedDistanceField& sdf,
                                                        const double resolution,
                                                        const PairGripperPositions& gripper_positions,
                                                        const Eigen::Vector3d& band_midpoint,
                                                        const double parabola_length, const bool gravity_aligned,
  ...
  const Isometry3d parabola_origin = [&] {
    ...
    if (gravity_aligned) {
      const Vector3d y_axis_temp =
          EigenHelpers::VectorProjectionToPlane(plane_vector1, plane_vector2, Eigen::Vector3d::UnitZ());
      ...
      origin.matrix() << x_axis, y_axis, z_axis, gripper_positions.first, 0.0, 0.0, 0.0, 1.0;
    }
...
sdf_tools::CollisionMapGrid ExtractParabolaSliceExtendDownwards(const sdf_tools::SignedDistanceField& sdf,
                                                                const double resolution,
                                                                const PairGripperPositions& gripper_positions,
                                                                const Eigen::Vector3d& band_midpoint,
                                                                const double parabola_length,
                                                                const bool gravity_aligned, const Visualizer::Ptr vis) {
  ...
  // Per matlab/wolfram
  const double numer =
      a2 * a - a2 * b + alpha * a - a * b2 - a * beta - a * l2 + b2 * b - alpha * b + b * beta - b * l2;
  const double denom = 2 * ((a - b) * (a - b) - l2);
  const double offset = a2 * a2 * l2 - 4 * a2 * a * b * l2 + 6 * a2 * b2 * l2 - 2 * a2 * l4 + 2 * a2 * alpha * l2 +
                        2 * a2 * beta * l2 - 4 * a * b2 * b * l2 + 4 * a * b * l4 - 4 * a * alpha * b * l2 -
                        4 * a * b * beta * l2 + b2 * b2 * l2 - 2 * b2 * l4 + 2 * alpha * b2 * l2 + 2 * b2 * beta * l2 +
                        l2 * l4 - 2 * alpha * l4 - 2 * beta * l4 + alpha * alpha * l2 - 2 * alpha * beta * l2 +
                        beta * beta * l2;
  const double z = (numer + std::sqrt(offset)) / denom;

  return ExtractParabolaSliceInPlane(sdf, resolution, gripper_positions,
                                     Vector3d(band_midpoint.x(), band_midpoint.y(), z), parabola_length,
                                     gravity_aligned, vis);
}

sdf_tools::CollisionMapGrid ExtractParabolaSlice(const sdf_tools::SignedDistanceField& sdf, const double resolution,
                                                 const PairGripperPositions& gripper_positions,
                                                 const RubberBand::ConstPtr& band,
                                                 const std::string& parabola_slice_option, const Visualizer::Ptr vis) {
  if (parabola_slice_option == "basic") {
    return ExtractParabolaSliceBasic(sdf, resolution, gripper_positions, band->maxSafeLength(), vis);
  }
  if (parabola_slice_option == "in_plane_gravity_aligned") {
    ...
    return ExtractParabolaSliceInPlane(sdf, resolution, gripper_positions, band_points[midpoint_idx],
                                       band->maxSafeLength(), true, vis);
  }
  if (parabola_slice_option == "in_plane_gripper_aligned") {
    const auto& band_points = band->upsampleBand();
    const auto midpoint_idx = band_points.size() / 2;
    return ExtractParabolaSliceInPlane(sdf, resolution, gripper_positions, band_points[midpoint_idx],
                                       band->maxSafeLength(), false, vis);
  }
  if (parabola_slice_option == "extend_downwards_gravity_aligned") {
    const auto& band_points = band->upsampleBand();
    const auto midpoint_idx = band_points.size() / 2;
    return ExtractParabolaSliceExtendDownwards(sdf, resolution, gripper_positions, band_points[midpoint_idx],
                                               band->maxSafeLength(), true, vis);
  }
