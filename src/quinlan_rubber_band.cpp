...
static std::vector<ssize_t> GetShortestPathBetweenGrippersThroughObject(
    const std::vector<GripperData>& grippers_data, const ObjectPointSet& object,
  ...
  const auto heuristic_fn = [&](const ssize_t& node) { return distance_fn(node, goal); };
  const auto goal_reached_fn = [&](const ssize_t& test_node) { return test_node == goal; };
  const auto astar_results = simple_astar_planner::SimpleAStarPlanner<ssize_t>::Plan(start, neighbour_fn, distance_fn,
                                                                                     heuristic_fn, goal_reached_fn);
...
ObjectPointSet QuinlanRubberBand::AggregateBandPoints(const std::vector<QuinlanRubberBand>& bands) {
  if (bands.size() == 0) {
  ...
  for (size_t band_idx = 0; band_idx < bands.size(); ++band_idx) {
    ...
    for (size_t point_idx = 0; point_idx < band_points.size(); ++point_idx) {
      points.col(band_idx * points_per_band + point_idx) = band_points[point_idx];
    }
  }
  return points;
}

ObjectPointSet QuinlanRubberBand::AggregateBandPoints(const std::vector<QuinlanRubberBand::Ptr>& bands) {
  if (bands.size() == 0) {
    return ObjectPointSet();
  }
...
ObjectPointSet QuinlanRubberBand::AggregateBandPoints(const std::vector<QuinlanRubberBand::ConstPtr>& bands) {
  if (bands.size() == 0) {
    return ObjectPointSet();
  }
...
ObjectPointSet QuinlanRubberBand::PointsFromBandPointsAndGripperTargets(
    const EigenHelpers::VectorVector3d& starting_points, const PairGripperPositions& grippers_targets,
...
ObjectPointSet QuinlanRubberBand::PointsFromBandAndGrippers(const QuinlanRubberBand& band,
...
QuinlanRubberBand::Ptr QuinlanRubberBand::BandFromNodeTransformsAndGrippers(
    const EigenHelpers::VectorIsometry3d& node_transforms, const PairGripperPositions& grippers_position,
...
QuinlanRubberBand::Ptr QuinlanRubberBand::BandFromWorldState(const WorldState& world_state,
                                                             const QuinlanRubberBand& template_band) {
  auto band = std::make_shared<QuinlanRubberBand>(template_band);
...
QuinlanRubberBand::QuinlanRubberBand(std::shared_ptr<ros::NodeHandle> nh, std::shared_ptr<ros::NodeHandle> ph,
                                     const Visualizer::Ptr vis, const sdf_tools::SignedDistanceField::ConstPtr& sdf,
                                     const XYZGrid& work_space_grid,
                                     const std::function<std::vector<ssize_t>(const ssize_t node)>& node_neighbours_fn,
                                     const WorldState& world_state, const double resample_max_pointwise_dist,
                                     const size_t upsample_num_points, const double max_safe_band_length)
    : nh_(nh),
      ...
      path_between_grippers_through_object_(GetShortestPathBetweenGrippersThroughObject(
          GetGrippersData(*nh_), GetObjectInitialConfiguration(*nh), node_neighbours_fn)),
      band_(),
      resampled_band_(),
...
bool QuinlanRubberBand::resetBand(const WorldState& world_state) {
  ...
  return resetBand(world_state.object_configuration_, ToGripperPositions(world_state.all_grippers_single_pose_));
}

bool QuinlanRubberBand::resetBand(const ObjectPointSet& object_config, const PairGripperPositions& gripper_positions) {
 ...
 * project the resampled points out of collision, but it should be feasible to do so without changing the "homotopy"
 * class
 ...
 * @return
 */
const EigenHelpers::VectorVector3d& QuinlanRubberBand::forwardPropagate(const PairGripperPositions& gripper_positions,
                                                                        bool verbose) {
  assert(bandIsValidWithVisualization());
...
const EigenHelpers::VectorVector3d& QuinlanRubberBand::getVectorRepresentation() const { return band_; }
...
const EigenHelpers::VectorVector3d& QuinlanRubberBand::resampleBand() const {
  ...
  if (resampled_band_.size() == 0) {
    const auto distance_fn = [](const Eigen::Vector3d& v1, const Eigen::Vector3d& v2) { return (v1 - v2).norm(); };
    resampled_band_ = path_utils::ResamplePath(band_, resample_max_pointwise_dist_, distance_fn,
                                               EigenHelpers::Interpolate<double, 3>);
  }
...
const EigenHelpers::VectorVector3d& QuinlanRubberBand::upsampleBand() const {
  ...
  if (upsampled_band_.size() == 0) {
    if (band_.size() == upsample_num_points_) {
      ...
      const auto distance_fn = [](const Eigen::Vector3d& v1, const Eigen::Vector3d& v2) { return (v1 - v2).norm(); };
      upsampled_band_ = path_utils::UpsamplePath<Eigen::Vector3d>(band_, upsample_num_points_, distance_fn,
                                                                  EigenHelpers::Interpolate<double, 3>);
    }
    // Also create the upsampled version here as a way to keep the code simple
    upsampled_band_single_vector_ = EigenHelpers::VectorEigenVectorToEigenVectorX(upsampled_band_);
  }
...
Pair3dPositions QuinlanRubberBand::getEndpoints() const { return {band_.front(), band_.back()}; }

double QuinlanRubberBand::maxSafeLength() const { return max_safe_band_length_; }

double QuinlanRubberBand::totalLength() const { return EigenHelpers::CalculateTotalDistance(band_); }

bool QuinlanRubberBand::isOverstretched() const { return totalLength() > max_safe_band_length_; }

std::vector<Visualizer::NamespaceId> QuinlanRubberBand::visualize(const std::string& marker_name,
...
std::vector<Visualizer::NamespaceId> QuinlanRubberBand::visualize(const EigenHelpers::VectorVector3d& test_band,
...
std::vector<Visualizer::NamespaceId> QuinlanRubberBand::visualizeWithBubbles(
    const std::string& marker_name, const std_msgs::ColorRGBA& safe_color,
    const std_msgs::ColorRGBA& overstretched_color, const int32_t id, const bool visualization_enabled) const {
  return visualizeWithBubbles(band_, marker_name, safe_color, overstretched_color, id, visualization_enabled);
}

std::vector<Visualizer::NamespaceId> QuinlanRubberBand::visualizeWithBubbles(
    const EigenHelpers::VectorVector3d& test_band, const std::string& marker_name,
    const std_msgs::ColorRGBA& safe_color, const std_msgs::ColorRGBA& overstretched_color, const int32_t id,
    const bool visualization_enabled) const {
  if (!visualization_enabled) {
    return {};
  }
    ...
    // Delete all sphere, markers, probably from just this publisher, and then republish
    {
      ...
      for (size_t idx = 0; idx < test_band.size(); ++idx) {
        ...
        colors[idx] = ColorBuilder::MakeFromFloatColors(
            (float)idx / (float)(test_band.size() - 1), 0.0f,
            (float)(test_band.size() - 1 - idx) / (float)(test_band.size() - 1), 0.3f);
      }
      const auto new_ids = vis_->visualizeSpheres(marker_name + "_bubbles", test_band, colors, id, bubble_sizes);
      vis_->forcePublishNow();
      marker_ids.insert(marker_ids.end(), std::make_move_iterator(new_ids.begin()),
...
std::vector<Visualizer::NamespaceId> QuinlanRubberBand::VisualizeBandSurface(const Visualizer::Ptr& vis,
...
std::vector<Visualizer::NamespaceId> QuinlanRubberBand::VisualizeBandSurface(
    const Visualizer::Ptr& vis, const std::vector<QuinlanRubberBand>& bands, const std_msgs::ColorRGBA& start_color,
    const std_msgs::ColorRGBA& end_color, const std::string& ns, const int32_t id) {
  if (bands.empty()) {
    return {};
  }
...
std::vector<Visualizer::NamespaceId> QuinlanRubberBand::VisualizeBandSurface(
    const Visualizer::Ptr& vis, const std::vector<QuinlanRubberBand::Ptr>& bands,
    const std_msgs::ColorRGBA& start_color, const std_msgs::ColorRGBA& end_color, const std::string& ns,
    const int32_t id) {
  if (bands.empty()) {
    return {};
  }
...
std::vector<Visualizer::NamespaceId> QuinlanRubberBand::VisualizeBandSurface(
    const Visualizer::Ptr& vis, const std::vector<QuinlanRubberBand::ConstPtr>& bands,
    const std_msgs::ColorRGBA& start_color, const std_msgs::ColorRGBA& end_color, const std::string& ns,
    const int32_t id) {
  if (bands.empty()) {
    return {};
...
Eigen::Vector3d QuinlanRubberBand::projectToValidBubble(const Eigen::Vector3d& location) const {
  ...
  const auto post_collision_project =
      sdf_->ProjectOutOfCollisionToMinimumDistance3d(location, min_distance_to_obstacle_);
  if (ENABLE_BAND_DEBUGGING_) {
    std::cout << "Projecting to valid volume" << std::endl;
  }
  const auto post_boundary_project =
      sdf_->ProjectIntoValidVolumeToMinimumDistance3d(post_collision_project, min_distance_to_obstacle_);

  if (ENABLE_BAND_DEBUGGING_) {
    std::cout << std::setprecision(20) << "location:       " << location.transpose() << std::endl
    ...
    if (getBubbleSize(post_boundary_project) < min_distance_to_obstacle_) {
      ...
      if (distance_to_boundary.first < min_distance_to_obstacle_ ||
        ...
        std::cerr << std::setprecision(p) << "Starting dist to obstacle: "
                  << PrettyPrint::PrettyPrint(starting_distance_to_obstacles, true, " ") << std::endl;
        std::cerr << std::setprecision(p)
                  << "Starting dist to boundary: " << PrettyPrint::PrettyPrint(starting_distance_to_boundary, true, " ")
                  << std::endl;

        std::cerr << std::setprecision(p)
                  << "Final dist to obstacle:    " << PrettyPrint::PrettyPrint(distance_to_obstacles, true, " ")
                  << std::endl;
        std::cerr << std::setprecision(p)
                  << "Final dist to boundary:    " << PrettyPrint::PrettyPrint(distance_to_boundary, true, " ")
                  << std::endl;

        std::cerr << std::setprecision(p) << "collision - location:      " << (post_collision_project - location).norm()
                  << std::endl;
        std::cerr << std::setprecision(p)
                  << "boundary - collision:      " << (post_boundary_project - post_collision_project).norm()
                  << std::endl;

        const auto post_collision_project_test =
            sdf_->ProjectOutOfCollisionToMinimumDistance3d(location, min_distance_to_obstacle_);
        const auto post_boundary_project_test =
            sdf_->ProjectIntoValidVolumeToMinimumDistance3d(post_collision_project_test, min_distance_to_obstacle_);
...
bool QuinlanRubberBand::sufficientOverlap(const double bubble_size_a, const double bubble_size_b,
                                          const double distance) const {
  return (bubble_size_a + bubble_size_b) >= (distance + min_overlap_distance_);
}

bool QuinlanRubberBand::bandIsValid() const { return bandIsValid(band_); }
...
bool QuinlanRubberBand::bandIsValid(const EigenHelpers::VectorVector3d& test_band) const {
  if (ENABLE_BAND_DEBUGGING_) {
    ...
    for (size_t node_idx = 0; node_idx < test_band.size() - 1; ++node_idx) {
      ...
      if (!sdf_->LocationInBounds3d(curr) || curr_bubble_size < min_distance_to_obstacle_ ||
...
bool QuinlanRubberBand::bandIsValidWithVisualization() const { return bandIsValidWithVisualization(band_); }
...
bool QuinlanRubberBand::bandIsValidWithVisualization(const EigenHelpers::VectorVector3d& test_band) const {
  if (ENABLE_BAND_DEBUGGING_) {
    if (!bandIsValid(test_band)) {
      visualizeWithBubbles(test_band, "quinlan_band_something_is_invalid", Visualizer::Black(), Visualizer::Cyan(), 1,
                           true);
...
bool QuinlanRubberBand::interpolateBetweenPoints(EigenHelpers::VectorVector3d& point_buffer,
                                                 const Eigen::Vector3d& target) const {
  if (ENABLE_BAND_DEBUGGING_ && ENABLE_INTERPOLATE_DEBUGGING_) {
    std::cout << PrettyPrint::PrettyPrint(point_buffer, true, "\n") << std::endl
              << "Start of interpolateBetweenPoints: " << std::flush;
    if (point_buffer.size() >= 2) {
      assert(bandIsValidWithVisualization(point_buffer));
    }
    if (point_buffer.size() > 1) {
      visualizeWithBubbles(point_buffer, "start_of_interpolateBetweenPoints", Visualizer::Blue(), Visualizer::Cyan(), 1,
                           true);
    }
  ...
  while (!sufficientOverlap(curr_bubble_size, target_bubble_size, distance_to_end)) {
    ...
    if (ENABLE_BAND_DEBUGGING_ && ENABLE_INTERPOLATE_DEBUGGING_ && outer_iteration_counter >= 20) {
      ...
      std::cerr << std::setprecision(12) << "Curr:   " << curr.transpose() << std::endl
      ...
      vis_->visualizeSpheres("interpolate_outer_debugging_curr_sphere", {curr}, Visualizer::Red(0.2f), 2,
                             curr_bubble_size);
      vis_->visualizePoint("interpolate_outer_debugging_interp_point", interpolated_point, Visualizer::Green(1.0f), 1,
                           0.005);
      vis_->visualizeSpheres("interpolate_outer_debugging_interp_sphere", {interpolated_point}, Visualizer::Green(0.2f),
                             2, interpolated_point_bubble_size);
      vis_->visualizePoint("interpolate_outer_debugging_test_point", test_point, Visualizer::Cyan(1.0f), 1, 0.005);
      vis_->visualizeSpheres("interpolate_outer_debugging_test_sphere", {test_point}, Visualizer::Cyan(0.2f), 2,
                             test_point_bubble_size);
      vis_->visualizePoint("interpolate_outer_debugging_target_point", target, Visualizer::Blue(1.0f), 1, 0.005);
      vis_->visualizeSpheres("interpolate_outer_debugging_target_sphere", {target}, Visualizer::Blue(0.2f), 2,
                             target_bubble_size);
    ...
    if (outer_iteration_counter == 20) {
      //                ROS_WARN_STREAM_NAMED("rubber_band", "Rubber band interpolation outer loop counter at " <<
      //                outer_iteration_counter << ", probably stuck in an infinite loop");
    ...
    while (!sufficientOverlap(curr_bubble_size, test_point_bubble_size, distance_between_curr_and_test_point)) {
      if (ENABLE_BAND_DEBUGGING_ && ENABLE_INTERPOLATE_DEBUGGING_) {
        ...
        if (inner_iteration_counter >= 5) {
          ...
          vis_->visualizeSpheres("interpolate_inner_debugging_curr_sphere", {curr}, Visualizer::Red(0.2f), 2,
                                 curr_bubble_size);
          vis_->visualizePoint("interpolate_inner_debugging_interp_point", interpolated_point, Visualizer::Green(1.0f),
                               1, 0.005);
          vis_->visualizeSpheres("interpolate_inner_debugging_interp_sphere", {interpolated_point},
                                 Visualizer::Green(0.2f), 2, interpolated_point_bubble_size);
          vis_->visualizePoint("interpolate_inner_debugging_test_point", test_point, Visualizer::Cyan(1.0f), 1, 0.005);
          vis_->visualizeSpheres("interpolate_inner_debugging_test_sphere", {test_point}, Visualizer::Cyan(0.2f), 2,
                                 test_point_bubble_size);
          vis_->visualizePoint("interpolate_inner_debugging_target_point", target, Visualizer::Blue(1.0f), 1, 0.005);
          vis_->visualizeSpheres("interpolate_inner_debugging_target_sphere", {target}, Visualizer::Blue(0.2f), 2,
                                 target_bubble_size);
          ...
          std::cerr << std::setprecision(12) << "Curr:   " << curr.transpose() << std::endl
                    ...
                    << "dist + min:         " << distance_between_curr_and_test_point + min_overlap_distance_
                    << std::endl
                    << std::endl;
      ...
      if (inner_iteration_counter == 5) {
        //                    ROS_WARN_STREAM_NAMED("rubber_band", "Rubber band interpolation inner loop counter at " <<
        //                    inner_iteration_counter << ", probably stuck in an infinite loop");
        //                    throw_arc_exception(std::runtime_error, "Interpolation stuck");
        return false;
      }
  ...
  if (ENABLE_BAND_DEBUGGING_ && ENABLE_INTERPOLATE_DEBUGGING_) {
    ...
    visualizeWithBubbles(point_buffer, "end_of_interpolate_between_points", Visualizer::Black(), Visualizer::Cyan(), 1,
                         true);
    assert(bandIsValidWithVisualization(point_buffer));
  ...
  if (ENABLE_BAND_DEBUGGING_ && ENABLE_INTERPOLATE_DEBUGGING_) {
    ...
    for (size_t idx = 0; idx < band_.size(); ++idx) {
      if (getBubbleSize(band_[idx]) < min_distance_to_obstacle_) {
        std::cerr << "idx: " << idx << " point: " << band_[idx].transpose() << " size: " << getBubbleSize(band_[idx])
                  << std::endl;
      }
    ...
    std::cout << PrettyPrint::PrettyPrint(band_, true, "\n") << std::endl
              << "Start of interpolateBandPoints: " << std::flush;
  }
...
void QuinlanRubberBand::removeExtraBandPoints(const bool verbose) {
  ...
  if (ENABLE_BAND_DEBUGGING_ && ENABLE_REMOVE_DEBUGGING_ && verbose) {
    vis_->visualizePoint("remove_extra_test_points_kept_points", forward_pass.back(), Visualizer::Cyan(1.0f),
                         (int32_t)forward_pass.size(), 0.002);
    vis_->visualizeSpheres("remove_extra_test_points_kept_spheres", {forward_pass.back()}, Visualizer::Cyan(0.2f),
                           (int32_t)forward_pass.size(), getBubbleSize(forward_pass.back()));
  }
  ...
  for (size_t curr_idx = 1; curr_idx + 1 < band_.size(); ++curr_idx) {
    ...
    if (prev.isApprox(curr) || next.isApprox(curr)) {
      if (ENABLE_BAND_DEBUGGING_ && ENABLE_REMOVE_DEBUGGING_ && verbose) {
        std::cout << "Removing point as it is really really really close to and adjacent one, idx: " << curr_idx
                  << std::endl;
      }
      continue;
    }
  ...
  if (ENABLE_BAND_DEBUGGING_ && ENABLE_REMOVE_DEBUGGING_ && verbose) {
    vis_->visualizePoints("remove_extra_test_points_kept_points", forward_pass.back(), Visualizer::Cyan(1.0f),
                          (int32_t)forward_pass.size(), 0.002);
    vis_->visualizeSpheres("remove_extra_test_points_kept_spheres", {forward_pass.back()}, Visualizer::Cyan(0.2f),
                           (int32_t)forward_pass.size(), getBubbleSize(forward_pass.back()));
  }
...
bool QuinlanRubberBand::smoothBandPoints(const bool verbose) {
  ...
  for (size_t smoothing_iter = 0; smoothing_iter < smoothing_iterations_; ++smoothing_iter) {
    ...
    for (size_t curr_idx = 1; curr_idx + 1 < band_.size(); ++curr_idx) {
      ...
      // Only step at most part way there in order to try and avoid oscillations - too large of a step size can be bad
      // due to errors in sdf->EstiamteDistance
      ...
      const Eigen::Vector3d prime = curr_plus_delta_inside_bubble
                                        ? Eigen::Vector3d(curr + delta)
                                        : Eigen::Vector3d(curr + max_delta_norm * delta.normalized());
      ...
      if (ENABLE_BAND_DEBUGGING_ && ENABLE_SMOOTHING_DEBUGGING_ && verbose) {
        if (projected_bubble_size < min_distance_to_obstacle_) {
          ...
          std::cout << std::setprecision(12) << "Projected: " << projected_testing.transpose() << std::endl
        ...
        vis_->visualizeSpheres("smoothing_test_projected", {projected}, Visualizer::Magenta(0.2f), 2,
                               projected_bubble_size);
        ...
        std::cout << std::setprecision(12) << "prev                      = [" << prev.transpose() << "]';\n"
                  << "next                      = [" << next.transpose() << "]';\n"
                  << std::endl
                  << "curr                      = [" << curr.transpose() << "]';\n"
                  ...
                  << std::endl
                  << "sdf_gradient =            = ["
                  << EigenHelpers::StdVectorDoubleToEigenVector3d(sdf_->GetGradient3d(curr)).transpose() << "]';\n"
        ...
        if (!curr_plus_delta_inside_bubble) {
          ...
          std::cout << "(SDF Est at curr) + delta_norm: " << sdf_->EstimateDistance3d(curr).first + delta.norm()
                    << std::endl;
          std::cout << "(SDF Est at curr) - delta_norm: " << sdf_->EstimateDistance3d(curr).first - delta.norm()
                    << std::endl;
...
void QuinlanRubberBand::printBandData(const EigenHelpers::VectorVector3d& test_band) const {
  if (ENABLE_BAND_DEBUGGING_) {
    ...
    std::cout << "SDF limits: x, y, z\n"
              ...
              << "Band limits: Min dist to obstacle: " << min_distance_to_obstacle_
              << "   Min overlap distance: " << min_overlap_distance_ << std::endl;
...
uint64_t QuinlanRubberBand::serialize(std::vector<uint8_t>& buffer) const {
  ...
  const auto bytes_written = arc_utilities::SerializeVector(band_, buffer, arc_utilities::SerializeEigen<double, 3, 1>);
...
std::pair<QuinlanRubberBand::Ptr, uint64_t> QuinlanRubberBand::Deserialize(const std::vector<uint8_t>& buffer,
...
void QuinlanRubberBand::loadStoredBand() {
  try {
    ...
    const auto file_name_suffix =
        ROSHelpers::GetParamRequiredDebugLog<std::string>(*ph_, "file_name_suffix_to_load", __func__);
...
bool QuinlanRubberBand::operator!=(const QuinlanRubberBand& other) const { return !(*this == other); }
...
double QuinlanRubberBand::Distance(const QuinlanRubberBand& b1, const QuinlanRubberBand& b2) { return b1.distance(b2); }
}  // namespace smmap
