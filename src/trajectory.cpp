...
std::pair<WorldState, uint64_t> WorldState::Deserialize(const std::vector<uint8_t>& buffer, const uint64_t current) {
  ...
  const auto deserialized_rope_node_transforms =
  ...
  const auto deserialized_robot_config_valid =
      arc_utilities::DeserializeFixedSizePOD<bool>(buffer, current + bytes_read);
  ...
  const WorldState result = {
      deserialized_object.first,       deserialized_rope_node_transforms.first, deserialized_grippers.first,
      deserialized_robot_config.first, deserialized_robot_config_valid.first,   deserialized_collision_data.first,
 ...
 * @return
 */
WorldState ConvertToEigenFeedback(const deformable_manipulation_msgs::WorldState& feedback_ros) {
  ...
  feedback_eigen.robot_configuration_ = EigenHelpers::StdVectorToEigenVectorX(feedback_ros.robot_configuration);
  ...
  for (size_t gripper_ind = 0; gripper_ind < num_grippers; gripper_ind++) {
    feedback_eigen.gripper_collision_data_.push_back(CollisionData(
        EigenHelpersConversions::GeometryPointToEigenVector3d(
            feedback_ros.gripper_nearest_point_to_obstacle[gripper_ind]),
        EigenHelpersConversions::GeometryVector3ToEigenVector3d(feedback_ros.obstacle_surface_normal[gripper_ind]),
 ...
 * @return
 */
AllGrippersPoseTrajectory GetGripperTrajectories(const std::vector<WorldState>& feedback) {
  AllGrippersPoseTrajectory grippers_trajectories(feedback.size());

  for (size_t time_ind = 0; time_ind < feedback.size(); time_ind++) {
    grippers_trajectories[time_ind] = feedback[time_ind].all_grippers_single_pose_;
  }

  return grippers_trajectories;
}

Eigen::VectorXd CalculateObjectDeltaAsVector(const ObjectPointSet& start, const ObjectPointSet& end) {
  Eigen::MatrixXd diff = end - start;
  diff.resize(diff.rows() * diff.cols(), 1);
  return diff;
}

ObjectPointSet AddObjectDelta(ObjectPointSet start, const Eigen::VectorXd& delta) {
