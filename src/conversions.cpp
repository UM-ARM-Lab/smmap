...
std::pair<std::vector<TransitionEstimation::StateMicrostepsPair>, bool> ToTrajectory(
    const WorldState& initial_world_state, const RRTPath& path, const dmm::TransitionTest& test,
    const dmm::TransitionTestResult& test_result) {
  static ros::NodeHandle nh;
  static const auto simsteps_per_gripper_cmd =
      ROSHelpers::GetParamRequiredDebugLog<int>(nh, "deform_simulator_node/num_simsteps_per_gripper_command", __func__)
          .Get();
  ...
  const auto path_total_substeps =
      std::accumulate(test.path_num_substeps.begin(), test.path_num_substeps.end(), test.final_num_substeps);
  ...
  // Add the first state with no history
  {
    const TransitionEstimation::State tes = {
        initial_world_state.object_configuration_, std::make_shared<RubberBand>(template_band),
        std::make_shared<RubberBand>(*path.front().band()), initial_world_state.rope_node_transforms_};
  ...
  for (size_t idx = 1; idx < path_num_steps; ++idx) {
    ...
    if ((test_result.microsteps_all.begin() + microsteps_start_idx >
         test_result.microsteps_all.end() - (test.final_num_substeps * simsteps_per_gripper_cmd)) ||
        (test_result.microsteps_all.begin() + microsteps_end_idx >
         test_result.microsteps_all.end() - (test.final_num_substeps * simsteps_per_gripper_cmd))) {
      ROS_WARN_STREAM_NAMED(
          "conversions", "Results only contains " << idx << " path steps. Path steps anticipated: " << path_num_steps);
      clean_trajectory = false;
      return {trajectory, clean_trajectory};
    }

    const std::vector<dmm::WorldState> dmm_microsteps(test_result.microsteps_all.begin() + microsteps_start_idx,
                                                      test_result.microsteps_all.begin() + microsteps_end_idx);
    const auto microsteps = ConvertToEigenFeedback(dmm_microsteps);

    try {
      if (!microsteps.back().object_configuration_.allFinite()) {
        ROS_WARN_STREAM_NAMED("conversions", "Input contains non-finite data at index "
                                                 << idx << ". Path steps anticipated: " << path_num_steps);
        clean_trajectory = false;
        return {trajectory, clean_trajectory};
      }
      const TransitionEstimation::State tes = {
      ...
      if (tes.rubber_band_->isOverstretched()) {
        ROS_WARN_STREAM_NAMED("conversions",
                              "Band overstretched at index " << idx << ". Path steps anticipated: " << path_num_steps);
        clean_trajectory = false;
        return {trajectory, clean_trajectory};
      }
  ...
  if (has_last_action) {
    ...
    try {
      if (end.object_configuration_.allFinite()) {
        ROS_WARN_STREAM_NAMED("conversions",
                              "Input contains non-finite data last action. Path steps anticipated: " << path_num_steps);
        clean_trajectory = false;
        return {trajectory, clean_trajectory};
      }
      const auto tes =
          TransitionEstimation::State{end.object_configuration_, RubberBand::BandFromWorldState(end, template_band),
                                      planned_band, end.rope_node_transforms_};
      if (tes.rubber_band_->isOverstretched()) {
