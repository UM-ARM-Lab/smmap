...
LeastSquaresJacobianModel::LeastSquaresJacobianModel(std::shared_ptr<ros::NodeHandle> nh,
                                                     const MatrixXd& initial_jacobian, const long extra_samples)
    : JacobianModel(nh),
...
void LeastSquaresJacobianModel::updateModel_impl(const WorldState& previous, const WorldState& next) {
  ...
  if (grippers_delta.squaredNorm() < 1e-6) {
    ROS_WARN_STREAM_NAMED("least_squares_jacobian", "Grippers did not move much, not updating: squared norm vel: "
  ...
  if (buffer_full_) {
    ...
    current_jacobian_ = deformable_delta_wide_matrix_ * Pinv(grippers_delta_wide_matrix_, SuggestedRcond());
  }
}
