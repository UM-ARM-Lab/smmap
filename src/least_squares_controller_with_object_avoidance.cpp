...
LeastSquaresControllerWithObjectAvoidance::LeastSquaresControllerWithObjectAvoidance(
    ...
    Visualizer::Ptr vis, const JacobianModel::ConstPtr& model, const double obstacle_avoidance_scale,
    const bool optimize)
...
DeformableController::OutputData LeastSquaresControllerWithObjectAvoidance::getGripperMotion_impl(
  ...
  // Remapped data array
  Map<VectorXd> object_delta_as_vector(suggested_robot_motion.object_motion_.data(),
                                       suggested_robot_motion.object_motion_.size());
  ...
  if (input_data.robot_jacobian_valid_) {
    ...
    for (size_t poi_ind = 0; poi_ind < num_poi; ++poi_ind) {
      ...
      linear_constraint_linear_terms[poi_ind] = -collision_data.obstacle_surface_normal_.transpose() * poi_jacobian;

      linear_constraint_affine_terms[poi_ind] =
          collision_data.distance_to_obstacle_ - robot_->min_controller_distance_to_obstacles_;

      //            std::cout << "Poi ind: " << poi_ind << " Dist to obstacle: " << collision_data.distance_to_obstacle_
      //            << " Min dist: " << robot_->min_controller_distance_to_obstacles_ << std::endl
    ...
    suggested_robot_motion.robot_dof_motion_ = minSquaredNorm_SE3VelocityConstraints_QuadraticConstraints(
        robot_dof_to_deformable_object_jacobian, desired_object_motion.delta, desired_object_motion.weight,
        robot_dof_to_grippers_poses_jacobian, max_grippers_step_size, linear_constraint_linear_terms,
    ...
    //        std::cout << "Pure gripper optimization: " <<
    //        grippers_delta_achieve_goal.head<6>().normalized().transpose() << "    " <<
    //        grippers_delta_achieve_goal.tail<6>().normalized().transpose() << std::endl; std::cout << "Full robot
    //        optimization: " << grippers_motion.head<6>().normalized().transpose() << "    " <<
    //        grippers_motion.tail<6>().normalized().transpose() << std::endl; std::cout << "Difference: "
    //                  << (grippers_delta_achieve_goal.head<6>().normalized() -
    //                  grippers_motion.head<6>().normalized()).transpose() << "    "
    //                  << (grippers_delta_achieve_goal.tail<6>().normalized() -
    //                  grippers_motion.tail<6>().normalized()).transpose() << std::endl;
    ...
    if (optimize_) {
      grippers_delta_achieve_goal =
          minSquaredNorm_SE3VelocityConstraints(grippers_poses_to_object_jacobian, desired_object_motion.delta,
                                                desired_object_motion.weight, max_grippers_step_size);
    } else {
      grippers_delta_achieve_goal = ClampGripperPoseDeltas(
          WeightedLeastSquaresSolver(grippers_poses_to_object_jacobian, desired_object_motion.delta,
                                     desired_object_motion.weight, LEAST_SQUARES_DAMPING_THRESHOLD,
                                     LEAST_SQUARES_DAMPING_VALUE),
          max_grippers_step_size);
    }

    // Find the collision avoidance data that we'll need
    const std::vector<CollisionAvoidanceResult> grippers_collision_avoidance_result = ComputeGripperObjectAvoidance(
        input_data.world_current_state_.gripper_collision_data_,
        input_data.world_current_state_.all_grippers_single_pose_, max_grippers_step_size);
    ...
    for (size_t gripper_ind = 0; gripper_ind < num_grippers; gripper_ind++) {
      suggested_robot_motion.grippers_motion_[gripper_ind] =
          CombineDesiredAndObjectAvoidance(grippers_delta_achieve_goal.segment<6>((ssize_t)gripper_ind * 6),
                                           grippers_collision_avoidance_result[gripper_ind], obstacle_avoidance_scale_);

      object_delta_as_vector += grippers_poses_to_object_jacobian.block(0, 6 * (ssize_t)gripper_ind, num_nodes * 3, 6) *
                                suggested_robot_motion.grippers_motion_[gripper_ind];
    }
  }
