...
DeformableModel::DeformableModel(std::shared_ptr<ros::NodeHandle> nh)
...
void DeformableModel::SetGrippersData(const std::vector<GripperData>& grippers_data) {
...
void DeformableModel::SetCallbackFunctions(const GripperCollisionCheckFunction& gripper_collision_check_fn) {
...
void DeformableModel::updateModel(const WorldState& previous, const WorldState& next) {
  return updateModel_impl(previous, next);
}

ObjectPointSet DeformableModel::getObjectDelta(const WorldState& world_state,
