...
StretchingConstraintController::StretchingConstraintController(
    std::shared_ptr<ros::NodeHandle> nh, std::shared_ptr<ros::NodeHandle> ph, RobotInterface::Ptr robot,
    Visualizer::Ptr vis, const DeformableModel::ConstPtr& model, const sdf_tools::SignedDistanceField::ConstPtr sdf,
    const std::shared_ptr<std::mt19937_64>& generator,
    const StretchingConstraintControllerSolverType gripper_controller_type, const int max_count)
...
DeformableController::OutputData StretchingConstraintController::solvedByRandomSampling(const InputData& input_data) {
  assert(false &&
         "Not updated to use new constraints etc. Verify that this whole function is doing what we want, for both "
         "simulation and live robot");
  ...
  const MatrixXd node_squared_distance = CalculateSquaredDistanceMatrix(current_world_state.object_configuration_);
...
#pragma omp parallel for
    ...
    for (int64_t ind_count = 0; ind_count < max_count_; ind_count++) {
      ...
      const bool collision_violation =
          gripperCollisionCheckResult(current_world_state.all_grippers_single_pose_, grippers_motion_sample);

      const bool stretching_violation = stretchingDetection(input_data, grippers_motion_sample);
      ...
      if (!collision_violation && !stretching_violation) {
        ...
        ObjectPointSet predicted_object_p_dot =
            model_->getObjectDelta(input_data.world_current_state_, grippers_motion_sample);

        const double sample_error =
            errorOfControlByPrediction(predicted_object_p_dot, desired_object_p_dot, desired_p_dot_weight);
    ...
    const ObjectPointSet object_motion =
        model_->getObjectDelta(input_data.world_current_state_, optimal_gripper_motion);

    const OutputData suggested_robot_command(optimal_gripper_motion, object_motion, VectorXd());
...
DeformableController::OutputData StretchingConstraintController::solvedByNomad(const InputData& input_data) {
  ...
  if (input_data.robot_jacobian_valid_) {
    ...
    const VectorXd min_joint_delta = distance_to_lower_joint_limits.unaryExpr(
        [&max_step_size](const double x) { return std::max(x, -max_step_size); });
    ...
    const VectorXd max_joint_delta = distance_to_upper_joint_limits.unaryExpr(
        [&max_step_size](const double x) { return std::min(x, max_step_size); });

    // Return value of objective function, cost = norm(p_dot_desired - p_dot_test)
    const std::function<double(const VectorXd&)> eval_error_cost_fn = [&](const VectorXd& test_robot_motion) {
      const VectorXd grippers_motion_as_single_vector = input_data.robot_jacobian_ * test_robot_motion;
      ...
      const ObjectPointSet predicted_object_p_dot =
          model_->getObjectDelta(input_data.world_current_state_, test_gripper_motion);

      return errorOfControlByPrediction(predicted_object_p_dot, desired_object_p_dot, desired_p_dot_weight);
    };
    ...
    const std::function<double(const VectorXd&)> collision_constraint_fn = [&](const VectorXd& test_robot_motion) {
      ...
      for (size_t poi_ind = 0; poi_ind < poi_collision_data.size(); ++poi_ind) {
        ...
        const double current_distance =
            initial_distance + collision_data.obstacle_surface_normal_.transpose() * poi_jacobian * test_robot_motion;
    ...
    const std::function<double(const VectorXd&)> stretching_constraint_fn = [&](const VectorXd& test_robot_motion) {
      const VectorXd grippers_motion_as_single_vector = input_data.robot_jacobian_ * test_robot_motion;
    ...
    const std::function<double(const VectorXd&)> robot_motion_constraint_fn = [&](const VectorXd& test_robot_motion) {
    ...
    const VectorXd optimal_robot_motion =
        minFunctionPointerDirectRobotDOF(log_file_path_, fix_step_, max_count_, num_dof, min_joint_delta,
                                         max_joint_delta, *generator_, uniform_unit_distribution_, eval_error_cost_fn,
                                         collision_constraint_fn, stretching_constraint_fn, robot_motion_constraint_fn);

    const VectorXd grippers_motion_as_single_vector = input_data.robot_jacobian_ * optimal_robot_motion;
    ...
    const OutputData suggested_robot_command(optimal_gripper_motion, object_motion, optimal_robot_motion);
    ...
    const std::function<double(const AllGrippersSinglePoseDelta&)> collision_constraint_fn =
        [&](const AllGrippersSinglePoseDelta& test_gripper_motion) {
          const double min_dis_to_obstacle =
              gripperCollisionCheckHelper(current_world_state.all_grippers_single_pose_, test_gripper_motion);
    ...
    const AllGrippersSinglePoseDelta optimal_gripper_motion = minFunctionPointerSE3Delta(
        log_file_path_, fix_step_, max_count_, num_grippers, max_step_size, *generator_, uniform_unit_distribution_,
        eval_error_cost_fn, collision_constraint_fn, stretching_constraint_fn, gripper_motion_constraint_fn);
...
inline VectorXd projectToMaxDeltaConstraints(const VectorXd& delta, const double max_step_size,
                                             const VectorXd& min_joint_delta, const VectorXd& max_joint_delta) {
...
inline double collisionConstraintFunction(const double b, const RowVectorXd& M, const VectorXd& dq) {
  return b - M * dq;
}

inline double barrier(const double u) { return -std::log(-u); }

inline double barrier(const double t, const double u) { return -(1.0 / t) * std::log(-u); }

VectorXd evaluateJointLimitsMaxDeltaConstraintsBarrier(const VectorXd& dq, const VectorXd& dq_min,
                                                       const VectorXd& dq_max, const double max_step_size,
                                                       const double t) {
...
VectorXd evaluateCollisionConstraintsBarrier(const std::vector<double>& b_offsets,
                                             const std::vector<RowVectorXd>& M_matrices, const VectorXd& dq,
                                             const double t) {
...
template <typename T>
VectorXd evaluateStretchingConstraintsBarrier(const AllGrippersSinglePoseDelta& gripper_motion,
                                              const T& stretching_evaluation_fn, const double t) {
  VectorXd result(1);
...
inline AllGrippersSinglePoseDelta stepInDirection(const AllGrippersSinglePoseDelta& start,
                                                  const AllGrippersSinglePoseDelta& direction, const double step_size) {
...
inline AllGrippersSinglePoseDelta stepInDirection(const AllGrippersSinglePoseDelta& start, const VectorXd& direction,
...
inline AllGrippersSinglePoseDelta stepInDirection(const AllGrippersSinglePoseDelta& start,
                                                  const std::vector<double>& direction, const double step_size) {
...
inline kinematics::Vector6d projectToCornerOfCollisionAndMaxDeltaConstraints(
    const kinematics::Vector6d& delta, const Matrix<double, 3, 6>& collision_jacobian, const double max_step_size) {
...
inline kinematics::Vector6d projectToCollisionAndMaxDeltaConstraints(const Isometry3d& starting_pose,
                                                                     const kinematics::Vector6d& delta,
                                                                     const CollisionData& collision_data,
                                                                     const double min_distance,
                                                                     const double max_step_size) {
  const Matrix<double, 3, 6> J_collision =
      ComputeGripperMotionToPointMotionJacobian(collision_data.nearest_point_to_obstacle_, starting_pose);
  ...
  // take the least violating motion (max speed, directly away from collision) as the result of projection
  {
    ...
    const double current_distance_change_needed =
        std::max(0.0, min_distance + GRIPPER_COLLISION_REPULSION_MARGIN - current_distance);
  ...
  // then we need to find the "corner" between the circle and the half plane
  {
    ...
    if (delta_norm > max_step_size) {
      ...
      if (current_distance >= min_distance) {
        ...
        const double current_distance_change_needed =
            min_distance + GRIPPER_COLLISION_REPULSION_MARGIN - current_distance;
        ...
        return projectToCornerOfCollisionAndMaxDeltaConstraints(collision_constrained_delta, J_collision,
                                                                max_step_size);
      }
    }
  }
...
inline AllGrippersSinglePoseDelta projectToCollisionAndMaxDeltaConstraints(
    const AllGrippersSinglePose& starting_poses, const AllGrippersSinglePoseDelta& delta,
    const std::vector<CollisionData>& collision_data, const double min_distance, const double max_step_size) {
  ...
  for (size_t ind = 0; ind < delta.size(); ++ind) {
    result[ind] = projectToCollisionAndMaxDeltaConstraints(starting_poses[ind], delta[ind], collision_data[ind],
                                                           min_distance, max_step_size);
  }
  return result;
}

DeformableController::OutputData StretchingConstraintController::solvedByGradientDescentProjectionViaGurobiMethod(
    ...
    for (ssize_t gripper_idx = 0; gripper_idx < num_grippers; ++gripper_idx) {
      ...
      if (true) {
        if (overstretch_) {
          ...
          Vector3d stretching_end = grippers_poses[gripper_idx] * (stretching_constraint_data[gripper_idx].second +
                                                                   0.1 * stretching_constraint_data[gripper_idx].first);

          //                std::cerr << "stretching_data_first_" << gripper_idx + 1 << " = [" <<
          //                stretching_constraint_data[gripper_idx].first.transpose() << " 0.0]';\n"; std::cerr <<
          //                "stretching_data_second_" << gripper_idx + 1 << " = [" <<
          //                stretching_constraint_data[gripper_idx].second.transpose() << " 1.0]';\n";

          //                std::cout << "stretching_start_" << gripper_idx + 1 << " = [" <<
          //                stretching_start.transpose() << "];\n"; std::cout << "stretching_end_" << gripper_idx + 1 <<
          //                " = [" << stretching_end.transpose() << "];\n";

          vis_->visualizeLines("stretching_correction_vector_" + std::to_string(gripper_idx), {stretching_start},
                               {stretching_end}, Visualizer::Red(), (int32_t)gripper_idx + 1);
          vis_->visualizePoints("stretching_correction_vector_" + std::to_string(gripper_idx),
                                VectorVector3d{stretching_start, stretching_end},
                                {Visualizer::Red(), Visualizer::Blue()}, (int32_t)(gripper_idx + num_grippers) + 1);
          visualizeCone(stretching_constraint_data[gripper_idx].first, stretching_cosine_threshold_,
                        grippers_poses[gripper_idx], (int32_t)gripper_idx + 1);
        } else {
  ...
  if (input_data.robot_jacobian_valid_) {
    ...
    // Collision constraints
    {
      ...
      for (size_t poi_ind = 0; poi_ind < num_poi; ++poi_ind) {
        ...
        linear_constraint_linear_terms.push_back(-collision_data.first.obstacle_surface_normal_.transpose() *
                                                 poi_jacobian);

        linear_constraint_affine_terms.push_back(collision_data.first.distance_to_obstacle_ -
                                                 robot_->min_controller_distance_to_obstacles_);

        //                std::cout << "Poi ind: " << poi_ind << " Dist to obstacle: " <<
        //                collision_data.first.distance_to_obstacle_ << " Min dist: " <<
        //                robot_->min_controller_distance_to_obstacles_ << std::endl
    ...
    if (joint_weights.size() == 16) {
      ROS_WARN_THROTTLE(4.0,
                        "Assuming that we are using Val, and each arm is to be treated independently, with the last 2 "
                        "DOF for the torso");
    ...
    ObjectPointSet object_delta =
        model_->getObjectDelta(world_state, RobotMotionToGripperMotion(jacobian, robot_motion));
    ...
    while (!converged) {
      ...
      for (ssize_t dof_ind = 0; dof_ind < num_dof; ++dof_ind) {
        ...
        const auto test_object_delta_plus_h =
            model_->getObjectDelta(world_state, RobotMotionToGripperMotion(jacobian, local_test_motion_plus_h));
        const auto test_error_plus_h =
            errorOfControlByPrediction(test_object_delta_plus_h, desired_object_p_dot, desired_p_dot_weight);

        VectorXd local_test_motion_minus_h = robot_motion;
        local_test_motion_minus_h(dof_ind) -= differencing_step_size;
        const auto test_object_delta_minus_h =
            model_->getObjectDelta(world_state, RobotMotionToGripperMotion(jacobian, local_test_motion_minus_h));
        const auto test_error_minus_h =
            errorOfControlByPrediction(test_object_delta_minus_h, desired_object_p_dot, desired_p_dot_weight);
      ...
      auto next_object_delta =
          model_->getObjectDelta(world_state, RobotMotionToGripperMotion(jacobian, next_robot_motion));
      ...
      do {
        ...
        next_robot_motion = findClosestValidPoint(
            potential_next_motion, jacobian, max_grippers_step_size, linear_constraint_linear_terms,
            linear_constraint_affine_terms, quadratic_constraint_quadratic_terms, quadratic_constraint_linear_terms,
            quadratic_constraint_affine_terms, min_joint_delta, max_joint_delta);
    ...
    // Collision constraints
    {
      const auto J_collision_g0 =
          ComputeGripperMotionToPointMotionJacobian(collision_data[0].nearest_point_to_obstacle_, grippers_poses[0]);
      ...
      linear_constraint_affine_terms_g0.push_back(robot_->min_controller_distance_to_obstacles_ -
                                                  collision_data[0].distance_to_obstacle_);

      const auto J_collision_g1 =
          ComputeGripperMotionToPointMotionJacobian(collision_data[1].nearest_point_to_obstacle_, grippers_poses[1]);
      ...
      linear_constraint_affine_terms_g1.push_back(robot_->min_controller_distance_to_obstacles_ -
                                                  collision_data[1].distance_to_obstacle_);
    }
    ...
    while (!converged) {
      ...
      for (ssize_t gripper_ind = 0; gripper_ind < num_grippers; ++gripper_ind) {
        for (size_t single_gripper_dir_ind = 0; single_gripper_dir_ind < 6; ++single_gripper_dir_ind) {
          ...
          //                    if (test_input_ind == 6)
          //                    {
          //                        vis_->visualizeObjectDelta("weirdness_gripper2_plus_x", object_config, object_config
          //                        + 20.0 * test_object_delta_plus_h, Visualizer::Cyan(), 1);
          //                    }

          //                    const auto test_object_delta = model_->getObjectDelta(world_state, local_test_motion);
          //                    const double test_error = errorOfControlByPrediction(test_object_delta,
          //                    desired_object_p_dot, desired_p_dot_weight); error_numerical_gradient(test_input_ind) =
          //                    test_error - error;
        }
      }
      ...
      //            std::cout << "Error gradient:                     0th: " <<
      //            error_numerical_gradient.head<6>().transpose() << "           1st: " <<
      //            error_numerical_gradient.tail<6>().transpose() << std::endl;
      ...
      double error_gradient_step_size = -initial_gradient_step_size;
      do {
        const auto potential_next_motion =
            stepInDirection(gripper_motion, error_numerical_gradient, error_gradient_step_size);

        //                const auto potential_next_object_delta = model_->getObjectDelta(world_state,
        //                potential_next_motion); const auto potential_next_error =
        //                errorOfControlByPrediction(potential_next_object_delta, desired_object_p_dot,
        //                desired_p_dot_weight);

        auto projected_g0_motion =
            findClosestValidPoint(potential_next_motion[0], linear_constraint_linear_terms_g0,
                                  linear_constraint_affine_terms_g0, max_individual_gripper_step_size);
        auto projected_g1_motion =
            findClosestValidPoint(potential_next_motion[1], linear_constraint_linear_terms_g1,
                                  linear_constraint_affine_terms_g1, max_individual_gripper_step_size);
...

        {
          ...
          //                {
          //                    manually_projected_g0_motion = potential_next_motion[0].normalized() *
          //                    max_individual_gripper_step_size; std::cout << "Manual projection max delta constraint
          //                    triggered\n";
          //                }
          ...
          //                {
          //                    manually_projected_g1_motion = potential_next_motion[1].normalized() *
          //                    max_individual_gripper_step_size; std::cout << "Manual projection max delta constraint
          //                    triggered\n";
          //                }
          //                auto manual_proj_object_delta = model_->getObjectDelta(world_state,
          //                {manually_projected_g0_motion, manually_projected_g1_motion}); auto manual_proj_next_error =
          //                errorOfControlByPrediction(manual_proj_object_delta, desired_object_p_dot,
          //                desired_p_dot_weight);
          ...
          //                auto collision_cnt_g0 = linear_constraint_linear_terms_g0[0] * potential_next_motion[0] +
          //                linear_constraint_affine_terms_g0[0]; auto collision_cnt_g1 =
          //                linear_constraint_linear_terms_g1[0] * potential_next_motion[1] +
          //                linear_constraint_affine_terms_g1[0];

          //                std::cout << "Collision constraint G0 before: " << collision_cnt_g0
          //                          << " Collision constraint G0 after:  " << linear_constraint_linear_terms_g0[0] *
          //                          projected_g0_motion + linear_constraint_affine_terms_g0[0] << std::endl;

          //                std::cout << "Collision constraint G1 before: " << collision_cnt_g1
          //                          << " Collision constraint G0 after:  " << linear_constraint_linear_terms_g1[0] *
          //                          projected_g1_motion + linear_constraint_affine_terms_g1[0] << std::endl;

          //                if (collision_cnt_g0 >= 0.0)
          //                {
          //                    Vector3d collision_start = grippers_poses[0].translation();
          //                    Vector3d collision_end = collision_start + grippers_poses[0].rotation() *
          //                    (J_distance_g0.head<3>().transpose());
          //                    vis_->visualizeLines("collision_avoidance_vector", {collision_start}, {collision_end},
          //                    Visualizer::Green(), 1); vis_->visualizeLines("collision_avoidance_vector",
          //                    {collision_start}, {collision_end}, Visualizer::Green(), 1);
          //                    vis_->visualizeLines("collision_avoidance_vector", {collision_start}, {collision_end},
          //                    Visualizer::Green(), 1); vis_->visualizeLines("collision_avoidance_vector",
          //                    {collision_start}, {collision_end}, Visualizer::Green(), 1);
          //                    vis_->visualizeLines("collision_avoidance_vector", {collision_start}, {collision_end},
          ...
          //                    Vector3d collision_end = collision_start + grippers_poses[1].rotation() *
          //                    (J_distance_g1.head<3>().transpose());
          //                    vis_->visualizeLines("collision_avoidance_vector", {collision_start}, {collision_end},
          //                    Visualizer::Green(), 2); vis_->visualizeLines("collision_avoidance_vector",
          //                    {collision_start}, {collision_end}, Visualizer::Green(), 2);
          //                    vis_->visualizeLines("collision_avoidance_vector", {collision_start}, {collision_end},
          //                    Visualizer::Green(), 2); vis_->visualizeLines("collision_avoidance_vector",
          //                    {collision_start}, {collision_end}, Visualizer::Green(), 2);
          //                    vis_->visualizeLines("collision_avoidance_vector", {collision_start}, {collision_end},
          ...
          //                std::cout << "Attempt ind: " << downhill_attempt_ind << "    Gradient step size: " <<
          //                error_gradient_step_size << std::endl;

          //                std::cout << "Error plus h change                 0th: " <<
          //                error_plus_h.head<6>().transpose() - kinematics::Vector6d::Ones().transpose() * error << "
          //                1st: " << error_plus_h.tail<6>().transpose() - kinematics::Vector6d::Ones().transpose() *
          //                error << std::endl; std::cout << "Error minus h change                0th: " <<
          //                error_minus_h.head<6>().transpose() - kinematics::Vector6d::Ones().transpose() * error << "
          //                1st: " << error_minus_h.tail<6>().transpose() - kinematics::Vector6d::Ones().transpose() *
          //                error << std::endl; std::cout << "Error gradient:                     0th: " <<
          //                error_numerical_gradient.head<6>().transpose()           << "           1st: " <<
          //                error_numerical_gradient.tail<6>().transpose() << std::endl; std::cout << "attempted delta:
          //                0th: " << delta.head<6>().transpose()                                    << "     1st: " <<
          //                delta.tail<6>().transpose() << std::endl; std::cout << "Gurobi based delta: 0th: " <<
          //                (projected_g0_motion - gripper_motion[0]).transpose()          << "     1st: " <<
          //                (projected_g1_motion - gripper_motion[1]).transpose() << std::endl; std::cout << "manual
          //                constraint based delta:      0th: " << (manually_projected_g0_motion -
          //                gripper_motion[0]).transpose() << "     1st: " << (manually_projected_g1_motion -
          //                gripper_motion[1]).transpose() << std::endl;
          ...
          //                std::cout << "Manual proj    :  " << print({manually_projected_g0_motion,
          //                manually_projected_g1_motion}) << std::endl;

          //                std::cout << "Starting norms:  " << GripperVelocity6dNorm(gripper_motion[0]) << " " <<
          //                GripperVelocity6dNorm(gripper_motion[0]) << std::endl; std::cout << "Potential norms: " <<
          //                GripperVelocity6dNorm(potential_next_motion[0]) << " " <<
          //                GripperVelocity6dNorm(potential_next_motion[0]) << std::endl; std::cout << "Gurobi   norms:
          //                " << GripperVelocity6dNorm(next_gripper_motion[0]) << " " <<
          //                GripperVelocity6dNorm(next_gripper_motion[0]) << std::endl; std::cout << "manual   norms:  "
          //                << GripperVelocity6dNorm(manually_projected_g0_motion) << " " <<
          //                GripperVelocity6dNorm(manually_projected_g0_motion) << std::endl;
          ...
          //                vis_->visualizeRope("gurobi_prediction", object_config + next_object_delta,
          //                Visualizer::Cyan(), 1); vis_->visualizeRope("gurobi_prediction", object_config +
          //                next_object_delta, Visualizer::Cyan(), 1); vis_->visualizeRope("gurobi_prediction",
          //                object_config + next_object_delta, Visualizer::Cyan(), 1);
          ...
          //                };
          //                vis_->visualizeLines("pre_projection_gripper_translation",
          //                potential_gripper_translation_starts, potential_gripper_translation_ends, Visualizer::Red(),
          //                1); vis_->visualizeLines("pre_projection_gripper_translation",
          //                potential_gripper_translation_starts, potential_gripper_translation_ends, Visualizer::Red(),
          //                1); vis_->visualizeLines("pre_projection_gripper_translation",
          //                potential_gripper_translation_starts, potential_gripper_translation_ends, Visualizer::Red(),
          //                1);
          ...
          //                };
          //                vis_->visualizeLines("gurobi_gripper_translation", gurobi_translation_starts,
          //                gubrobi_translation_ends, Visualizer::Blue(), 1);
          //                vis_->visualizeLines("gurobi_gripper_translation", gurobi_translation_starts,
          //                gubrobi_translation_ends, Visualizer::Blue(), 1);
          //                vis_->visualizeLines("gurobi_gripper_translation", gurobi_translation_starts,
          //                gubrobi_translation_ends, Visualizer::Blue(), 1);
...
kinematics::Vector6d StretchingConstraintController::singleGripperPoseDeltaSampler(const double max_delta) {
...
AllGrippersSinglePoseDelta StretchingConstraintController::allGripperPoseDeltaSampler(const ssize_t num_grippers,
...
kinematics::Vector6d StretchingConstraintController::getConstraintAwareGripperDeltaSample(
    const Isometry3d& gripper_pose, const CollisionData& collision_data, const double max_delta,
    const std::pair<Vector3d, Vector3d>& stretching_correction_data) {
  const auto J_collision =
      ComputeGripperMotionToPointMotionJacobian(collision_data.nearest_point_to_obstacle_, gripper_pose);
  const auto J_distance = collision_data.obstacle_surface_normal_.transpose() * J_collision;
  // Returns true if the constraint is satisfied
  const auto collision_constraint_fn = [&](const kinematics::Vector6d& gripper_delta) {
    return robot_->min_controller_distance_to_obstacles_ - collision_data.distance_to_obstacle_ -
           J_distance * gripper_delta;
  };
  ...
  //    vis_->visualizeLines("stretching_correction_vector", {stretching_start}, {stretching_end}, Visualizer::Red(),
  //    1);
  ...
  while (!valid_sample) {
    ...
    //        std::cout << "Stretching constraint: " << evaluateStretchingConstraint(stretching_correction_data, sample)
    //        << std::endl;
...
kinematics::Vector6d StretchingConstraintController::getFeasibleGripperDeltaGurobi(
  ...
  linear_constraint_affine_terms.push_back(collision_data.distance_to_obstacle_ -
                                           robot_->min_controller_distance_to_obstacles_ -
                                           GRIPPER_COLLISION_REPULSION_MARGIN);
  ...
  if (overstretch_) {
    const auto pyramid_plane_normals =
        ConvertConeToPyramid(stretching_correction_data.first, stretching_cosine_threshold_);
    ...
    for (size_t ind = 0; ind < pyramid_plane_normals.size(); ++ind) {
      std::cout << "Testing plane normal, should be negative: "
                << pyramid_plane_normals[ind].transpose() * stretching_correction_data.first << std::endl;
  ...
  std::cout << "Collision constraint val:  "
            << robot_->min_controller_distance_to_obstacles_ - collision_data.distance_to_obstacle_ -
                   J_distance * potential_feasible_point.first
            << std::endl;
  std::cout << "Stretching constraint val: "
            << evaluateStretchingConstraint(stretching_correction_data, potential_feasible_point.first) << std::endl;
  ...
  if (potential_feasible_point.second >= 0.0) {
    //        std::cout <<
    //        auto tmp = Pinv(J_distance, SuggestedRcond()) * (robot_->min_controller_distance_to_obstacles_ -
...
double StretchingConstraintController::errorOfControlByPrediction(const ObjectPointSet predicted_object_p_dot,
...
void StretchingConstraintController::visualize_stretching_vector(const ObjectPointSet& object_configuration) {
...
void StretchingConstraintController::visualize_rope_stretching_vector(const ObjectPointSet& object_configuration) {
  ...
  Vector3d first_correction_vector =
      (object_configuration.block<3, 1>(0, start_node + 1) - object_configuration.block<3, 1>(0, start_node));
  first_correction_vector = first_correction_vector / first_correction_vector.norm();

  Vector3d second_correction_vector =
      (object_configuration.block<3, 1>(0, end_node - 1) - object_configuration.block<3, 1>(0, end_node));
  ...
  vis_->visualizeLines("gripper overstretch motion", line_starts, line_ends, Visualizer::Orange());
}

void StretchingConstraintController::visualize_cloth_stretching_vector(const ObjectPointSet& object_configuration) {
  ...
  for (size_t stretching_ind = 0; stretching_ind < first_from_nodes.size(); stretching_ind++) {
    first_correction_vector +=
        first_contribution[stretching_ind] * (object_configuration.block<3, 1>(0, first_to_nodes[stretching_ind]) -
  ...
  for (size_t stretching_ind = 0; stretching_ind < second_from_nodes.size(); stretching_ind++) {
    second_correction_vector +=
        second_contribution[stretching_ind] * (object_configuration.block<3, 1>(0, second_to_nodes[stretching_ind]) -
...
void StretchingConstraintController::visualize_gripper_motion(const AllGrippersSinglePose& current_gripper_pose,
  ...
  for (size_t gripper_ind = 0; gripper_ind < current_gripper_pose.size(); gripper_ind++) {
    ...
    line_ends.push_back(
        ...
        100 * (grippers_test_poses[gripper_ind].translation() - current_gripper_pose[gripper_ind].translation()));
  }

  vis_->visualizeLines("gripper motion", line_starts, line_ends, Visualizer::Olive());
}

double StretchingConstraintController::gripperCollisionCheckHelper(
    const AllGrippersSinglePose& current_gripper_pose, const AllGrippersSinglePoseDelta& test_gripper_motion) const {
...
bool StretchingConstraintController::stretchingDetection(const InputData& input_data,
                                                         const AllGrippersSinglePoseDelta& test_gripper_motion) {
...
double StretchingConstraintController::stretchingFunctionEvaluation(
    const InputData& input_data, const AllGrippersSinglePoseDelta& test_gripper_motion) {
  //    assert(false && "This function is not used for the gradient descent method");
...
double StretchingConstraintController::ropeTwoGripperStretchingHelper(
    const InputData& input_data, const AllGrippersSinglePoseDelta& test_gripper_motion) {
  //    assert(false && "This function is not used for the gradient descent method" && "Verify the math in this
  //    implementation");
  ...
  if (overstretch_) {
    ...
    switch (gripper_controller_type_) {
      case StretchingConstraintControllerSolverType::RANDOM_SAMPLING: {
        if (sample_count_ > -1) {
          for (size_t gripper_ind = 0; gripper_ind < current_gripper_pose.size(); gripper_ind++) {
            Vector3d resulting_gripper_motion =
                grippers_test_poses[gripper_ind].translation() - current_gripper_pose[gripper_ind].translation();
      ...
      default: {
        for (size_t gripper_ind = 0; gripper_ind < current_gripper_pose.size(); gripper_ind++) {
          Vector3d resulting_gripper_motion =
              grippers_test_poses[gripper_ind].translation() - current_gripper_pose[gripper_ind].translation();
          stretching_sum +=
              resulting_gripper_motion.dot(stretching_correction_vector[gripper_ind]) / resulting_gripper_motion.norm();
        }
        stretching_cos = stretching_sum / (double)(current_gripper_pose.size());
        break;
      }
...
double StretchingConstraintController::clothTwoGripperStretchingHelper(
  ...
  const Vector3d first_point_in_first_gripper_frame =
      gripper0_current_pose.inverse() * point_on_first_gripper_before_motion;
  const Vector3d second_point_in_second_gripper_frame =
      gripper1_current_pose.inverse() * point_on_second_gripper_before_motion;
  ...
  if ((sample_count_ > -1) && (gripper_controller_type_ == StretchingConstraintControllerSolverType::RANDOM_SAMPLING)) {
    ...
    for (size_t gripper_ind = 0; gripper_ind < current_grippers_pose.size(); gripper_ind++) {
      std::cout << "point direction: " << point_motion_vector[gripper_ind].transpose()
                << "    stretching_correction_vec: "
                << per_gripper_stretching_correction_vector[gripper_ind].transpose() << std::endl;
  ...
  if (std::isnan(stretching_cos)) {
    ...
    std::cout << "trans0: "
              << (grippers_test_poses[0].translation() - current_grippers_pose[0].translation()).transpose()
              << std::endl;
    std::cout << "trans1: "
              << (grippers_test_poses[1].translation() - current_grippers_pose[1].translation()).transpose()
              << std::endl;
...
double StretchingConstraintController::evaluateStretchingConstraint(
    const std::pair<Vector3d, Vector3d>& stretching_constraint_data, const kinematics::Vector6d& gripper_delta) const {
  ...
  if (!overstretch_) {
    ...
    const Vector3d r_dot =
        gripper_delta.head<3>() + gripper_delta.tail<3>().cross(vector_from_gripper_to_translation_point);
...
std::vector<std::pair<Vector3d, Vector3d>> StretchingConstraintController::stretchingCorrectionVectorsAndPoints(
...
StretchingConstraintController::ropeTwoGrippersStretchingCorrectionVectorsAndPoints(const InputData& input_data) const {
...
std::vector<std::pair<Vector3d, Vector3d>>
StretchingConstraintController::clothTwoGrippersStretchingCorrectionVectorsAndPoints(
    const InputData& input_data) const {
  // Assume knowing there are two grippers.
  assert(grippers_data_.size() == 2 || "grippers size is not 2, stretching vectors not defined");
...
void StretchingConstraintController::visualizeCone(const Vector3d& cone_direction,
                                                   const double min_normalized_dot_product, const Isometry3d& pose,
                                                   const int marker_id) {
  ...
  const VectorVector3d ends = {pose * (ray1.normalized() / 10.0), pose * (ray2.normalized() / 10.0),
                               pose * (ray3.normalized() / 10.0), pose * (ray4.normalized() / 10.0)};
