...
ClothColabFolding::ClothColabFolding(std::shared_ptr<ros::NodeHandle> nh, std::shared_ptr<ros::NodeHandle> ph,
                                     Visualizer::Ptr vis)
...
double ClothColabFolding::calculateError_impl(const WorldState& world_state) {
  ...
  for (std::map<long, long>::const_iterator ittr = mirror_map_.begin(); ittr != mirror_map_.end(); ++ittr) {
    error +=
        (current_configuration.col(ittr->second) - point_reflector_.reflect(current_configuration.col(ittr->first)))
            .norm();
  }
...
ObjectDeltaAndWeight ClothColabFolding::calculateObjectErrorCorrectionDelta_impl(const WorldState& world_state) {
  ...
  for (std::map<long, long>::const_iterator ittr = mirror_map_.begin(); ittr != mirror_map_.end();
       ++ittr, ++robot_cloth_points_ind) {
...
PointReflector ClothColabFolding::CreatePointReflector(ros::NodeHandle& nh) {
  ...
  return PointReflector(mirror_line_data.response.mid_x, mirror_line_data.response.min_y,
...
std::map<long, long> ClothColabFolding::CreateMirrorMap(ros::NodeHandle& nh, const PointReflector& point_reflector) {
  ...
  for (long node_ind = 0; node_ind < object_initial_configuration.cols(); node_ind++) {
    ...
    if (object_initial_configuration(0, node_ind) > point_reflector.get_mid_x()) {
      const long mirror_ind = EigenHelpers::ClosestPointInSet(
          object_initial_configuration, point_reflector.reflect(object_initial_configuration.col(node_ind)));
...
bool ClothColabFolding::taskDone_impl(const WorldState& world_state) {
  (void)world_state;
  return false;
}
...
RopeDirectCoverage::RopeDirectCoverage(std::shared_ptr<ros::NodeHandle> nh, std::shared_ptr<ros::NodeHandle> ph,
                                       Visualizer::Ptr vis)
    : DirectCoverageTask(nh, ph, vis), neighbours_(num_nodes_) {}

std::vector<ssize_t> RopeDirectCoverage::getNodeNeighbours_impl(const ssize_t node) const {
  return neighbours_.getNodeNeighbours(node);
}

bool RopeDirectCoverage::taskDone_impl(const WorldState& world_state) {
...
ClothDirectCoverage::ClothDirectCoverage(std::shared_ptr<ros::NodeHandle> nh, std::shared_ptr<ros::NodeHandle> ph,
                                         Visualizer::Ptr vis)
    : DirectCoverageTask(nh, ph, vis), neighbours_(num_nodes_, GetClothNumControlPointsX(*nh_)) {}

std::vector<ssize_t> ClothDirectCoverage::getNodeNeighbours_impl(const ssize_t node) const {
  return neighbours_.getNodeNeighbours(node);
}

bool ClothDirectCoverage::taskDone_impl(const WorldState& world_state) {
  return calculateError(world_state) < error_threshold_task_done_;
}
...
RopeDistanceBasedCorrespondences::RopeDistanceBasedCorrespondences(std::shared_ptr<ros::NodeHandle> nh,
                                                                   std::shared_ptr<ros::NodeHandle> ph,
                                                                   Visualizer::Ptr vis)
    : DistanceBasedCorrespondencesTask(nh, ph, vis), neighbours_(num_nodes_) {}

std::vector<ssize_t> RopeDistanceBasedCorrespondences::getNodeNeighbours_impl(const ssize_t node) const {
  return neighbours_.getNodeNeighbours(node);
}

bool RopeDistanceBasedCorrespondences::taskDone_impl(const WorldState& world_state) {
  return calculateError(world_state) < error_threshold_task_done_;
}
...
ClothDistanceBasedCorrespondences::ClothDistanceBasedCorrespondences(std::shared_ptr<ros::NodeHandle> nh,
                                                                     std::shared_ptr<ros::NodeHandle> ph,
                                                                     Visualizer::Ptr vis)
    : DistanceBasedCorrespondencesTask(nh, ph, vis), neighbours_(num_nodes_, GetClothNumControlPointsX(*nh_)) {
...
bool ClothDistanceBasedCorrespondences::taskDone_impl(const WorldState& world_state) {
  return calculateError(world_state) < error_threshold_task_done_;
}
...
RopeFixedCorrespondences::RopeFixedCorrespondences(std::shared_ptr<ros::NodeHandle> nh,
                                                   std::shared_ptr<ros::NodeHandle> ph, Visualizer::Ptr vis)
    : FixedCorrespondencesTask(nh, ph, vis), neighbours_(num_nodes_) {
...
bool RopeFixedCorrespondences::taskDone_impl(const WorldState& world_state) {
  return calculateError(world_state) < error_threshold_task_done_;
}
...
ClothFixedCorrespondences::ClothFixedCorrespondences(std::shared_ptr<ros::NodeHandle> nh,
                                                     std::shared_ptr<ros::NodeHandle> ph, Visualizer::Ptr vis)
    : FixedCorrespondencesTask(nh, ph, vis), neighbours_(num_nodes_, GetClothNumControlPointsX(*nh_)) {
  assert(num_nodes_ == num_cover_points_);
...
bool ClothFixedCorrespondences::taskDone_impl(const WorldState& world_state) {
  return calculateError(world_state) < error_threshold_task_done_;
}
