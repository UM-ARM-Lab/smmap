...
LeastSquaresControllerWithStretchingConstraint::LeastSquaresControllerWithStretchingConstraint(
    ...
    Visualizer::Ptr vis, const JacobianModel::ConstPtr& model)
...
DeformableController::OutputData LeastSquaresControllerWithStretchingConstraint::getGripperMotion_impl(
      ...
      if (visualize_overstretch_cones_) {
        if (overstretch) {
          ...
          const Vector3d stretching_end =
              grippers_poses[gripper_idx] *
              (stretching_constraint_data[gripper_idx].second + 0.1 * stretching_constraint_data[gripper_idx].first);
  ...
  if (input_data.robot_jacobian_valid_) {
    ...
    // Collision constraints, in the form Cx <= d
    {
      ...
      linear_constraint_affine_terms.push_back(collision_data[0].distance_to_obstacle_ -
      ...
      linear_constraint_affine_terms.push_back(collision_data[1].distance_to_obstacle_ -
                                               robot_->min_controller_distance_to_obstacles_);
    ...
    const VectorXd grippers_motion = minSquaredNorm_SE3VelocityConstraints_LinearConstraints(
        grippers_poses_to_object_jacobian, desired_object_p_dot, desired_p_dot_weight,
        input_data.max_grippers_step_size_, linear_constraint_linear_terms, linear_constraint_affine_terms);

    object_delta_as_vector = grippers_poses_to_object_jacobian * grippers_motion;

    //        std::cout << linear_constraint_linear_terms[1] * grippers_motion << "   shall be <=    " <<
    //        linear_constraint_affine_terms[1] << std::endl;
...
LeastSquaresControllerWithStretchingConstraint::stretchingCorrectionVectorsAndPoints(
...
std::vector<std::pair<Vector3d, Vector3d>>
LeastSquaresControllerWithStretchingConstraint::ropeTwoGrippersStretchingCorrectionVectorsAndPoints(
    const InputData& input_data) const {
...
std::vector<std::pair<Vector3d, Vector3d>>
LeastSquaresControllerWithStretchingConstraint::clothTwoGrippersStretchingCorrectionVectorsAndPoints(
    const InputData& input_data) const {
...
void LeastSquaresControllerWithStretchingConstraint::visualizeCone(const Vector3d& cone_direction,
                                                                   const double min_normalized_dot_product,
                                                                   const Isometry3d& pose, const int marker_id) {
