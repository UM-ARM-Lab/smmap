...
TaskSpecification::Ptr TaskSpecification::MakeTaskSpecification(std::shared_ptr<ros::NodeHandle> nh,
  ...
  switch (task_type) {
    ...
    default:
      throw_arc_exception(std::invalid_argument,
                          "Invalid task type in MakeTaskSpecification(), this should not be possible");
      return nullptr;
  }
}
...
TaskSpecification::TaskSpecification(std::shared_ptr<ros::NodeHandle> nh, std::shared_ptr<ros::NodeHandle> ph,
                                     Visualizer::Ptr vis, const bool is_dijkstras_type_task)
...
std::vector<Visualizer::NamespaceId> TaskSpecification::visualizeDeformableObject(
    const std::string& marker_name, const ObjectPointSet& object_configuration, const std_msgs::ColorRGBA& color,
    const int32_t id) const {
  return visualizeDeformableObject_impl(marker_name, object_configuration, color, id);
}

std::vector<Visualizer::NamespaceId> TaskSpecification::visualizeDeformableObject(
    const std::string& marker_name, const ObjectPointSet& object_configuration,
    const std::vector<std_msgs::ColorRGBA>& colors, const int32_t id) const {
...
double TaskSpecification::calculateError(const WorldState& world_state) {
  ...
  if (current_error_calculated_.load()) {
    ...
    if (current_error_calculated_.load()) {
      ...
      ROS_INFO_STREAM_NAMED("task_specification",
                            "Calculated error in                  " << GlobalStopwatch(READ) << " seconds");
...
ObjectDeltaAndWeight TaskSpecification::calculateObjectErrorCorrectionDelta(const WorldState& world_state) {
  return calculateObjectErrorCorrectionDelta_impl(world_state);
}

bool TaskSpecification::taskDone(const WorldState& world_state) { return taskDone_impl(world_state); }

////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// Helper functions
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

double TaskSpecification::defaultDeformability() const { return default_deformability_; }

double TaskSpecification::collisionScalingFactor() const { return collision_scaling_factor_; }

double TaskSpecification::maxStretchFactor() const { return max_stretch_factor_; }

double TaskSpecification::maxBandLength() const { return max_band_length_; }

double TaskSpecification::maxTime() const { return max_time_; }

inline void addStrechingCorrectionVector(ObjectDeltaAndWeight& stretching_correction,
                                         const ObjectPointSet& object_configuration, const ssize_t first_node,
                                         const ssize_t second_node, const double node_distance_delta) {
  // The correction vector points from the first node to the second node,
  // and is half the length of the "extra" distance
  const Eigen::Vector3d correction_vector =
      0.5 * node_distance_delta * (object_configuration.col(second_node) - object_configuration.col(first_node));
  ...
  const double first_node_max_stretch = std::max(stretching_correction.weight(3 * first_node),
                                                 STRETCHING_WEIGHT_MULTIPLICATION_FACTOR * node_distance_delta);
  ...
  // Set the weight to be the stretch distance of the worst offender
  const double second_node_max_stretch = std::max(stretching_correction.weight(3 * second_node),
                                                  STRETCHING_WEIGHT_MULTIPLICATION_FACTOR * node_distance_delta);
...
bool TaskSpecification::stretchingConstraintViolated(const ssize_t first_node_ind, const Eigen::Vector3d& first_node,
...
ObjectDeltaAndWeight TaskSpecification::calculateStretchingCorrectionDeltaFullyConnected(
    const ObjectPointSet& object_configuration, bool visualize) const {
  ...
  for (ssize_t first_node = 0; first_node < num_nodes_; ++first_node) {
    for (ssize_t second_node = first_node + 1; second_node < num_nodes_; ++second_node) {
      ...
      if (max_dist < dist) {
        addStrechingCorrectionVector(stretching_correction, object_configuration, first_node, second_node,
 ...
 * @return
 */
ObjectDeltaAndWeight TaskSpecification::calculateStretchingCorrectionDelta(const WorldState& world_state,
...
ObjectDeltaAndWeight TaskSpecification::combineErrorCorrectionAndStretchingCorrection(
    const ObjectDeltaAndWeight& error_correction, const ObjectDeltaAndWeight& stretching_correction) const {
  ObjectDeltaAndWeight combined(num_nodes_ * 3);

  for (ssize_t node_ind = 0; node_ind < num_nodes_ * 3; node_ind += 3) {
    const Eigen::Vector3d error_correction_perpendicular = EigenHelpers::VectorRejection(
        stretching_correction.delta.segment<3>(node_ind), error_correction.delta.segment<3>(node_ind));

    combined.delta.segment<3>(node_ind) =
        stretching_correction.delta.segment<3>(node_ind) + error_correction_perpendicular;
    combined.weight.segment<3>(node_ind) =
        stretching_correction.weight.segment<3>(node_ind) + error_correction.weight.segment<3>(node_ind);
  }
...
DesiredDirection TaskSpecification::calculateDesiredDirection(const WorldState& world_state) {
  ...
  if (first_step_calculated_.load()) {
    ...
    if (first_step_calculated_.load()) {
      ...
      ROS_INFO_STREAM_NAMED("task_specification",
                            "Found best error correction delta in " << GlobalStopwatch(READ) << " seconds");

      GlobalStopwatch(RESET);
      const bool visualize_stretching_lines = false;
      first_step_desired_motion_.stretching_correction_ =
          calculateStretchingCorrectionDelta(world_state, visualize_stretching_lines);
      ROS_INFO_STREAM_NAMED("task_specification",
                            "Found stretching correction delta in " << GlobalStopwatch(READ) << " seconds");
      ...
      ROS_INFO_STREAM_NAMED("task_specification",
                            "Combined deltas in                   " << GlobalStopwatch(READ) << " seconds");
...
ModelAccuracyTestTask::ModelAccuracyTestTask(std::shared_ptr<ros::NodeHandle> nh, std::shared_ptr<ros::NodeHandle> ph,
                                             Visualizer::Ptr vis)
    : TaskSpecification(nh, ph, vis, false) {}

double ModelAccuracyTestTask::calculateError_impl(const WorldState& world_state) {
  (void)world_state;
  return 0.0;
}

ObjectDeltaAndWeight ModelAccuracyTestTask::calculateObjectErrorCorrectionDelta_impl(const WorldState& world_state) {
  (void)world_state;
...
bool ModelAccuracyTestTask::taskDone_impl(const WorldState& world_state) {
  (void)world_state;
  return false;
}
...
CoverageTask::CoverageTask(std::shared_ptr<ros::NodeHandle> nh, std::shared_ptr<ros::NodeHandle> ph,
                           Visualizer::Ptr vis, const bool is_dijkstras_type_task)
    : TaskSpecification(nh, ph, vis, is_dijkstras_type_task),
      sdf_(GetEnvironmentSDF(*nh_)),
      work_space_grid_(sdf_->GetOriginTransform(), sdf_->GetFrame(), GetWorldXStep(*nh_), GetWorldYStep(*nh_),
                       GetWorldZStep(*nh_), GetWorldXNumSteps(*nh_), GetWorldYNumSteps(*nh_), GetWorldZNumSteps(*nh_)),
...
bool CoverageTask::pointIsCovered(const ssize_t cover_idx, const Eigen::Vector3d& test_point) const {
  ...
  return (distances.first < error_threshold_distance_to_normal_) &&
         (std::abs(distances.second) < error_threshold_along_normal_);
}
...
DirectCoverageTask::DirectCoverageTask(std::shared_ptr<ros::NodeHandle> nh, std::shared_ptr<ros::NodeHandle> ph,
                                       Visualizer::Ptr vis)
...
ObjectDeltaAndWeight DirectCoverageTask::calculateObjectErrorCorrectionDelta_impl(const WorldState& world_state) {
 ...
 */
std::tuple<ssize_t, double, bool> DirectCoverageTask::findNearestObjectPoint(const WorldState& world_state,
...
DijkstrasCoverageTask::DijkstrasCoverageTask(std::shared_ptr<ros::NodeHandle> nh, std::shared_ptr<ros::NodeHandle> ph,
                                             Visualizer::Ptr vis)
...
#pragma omp parallel for schedule(guided)
    for (size_t cover_ind = 0; cover_ind < (size_t)num_cover_points_; ++cover_ind) {
      ...
      const auto result = arc_dijkstras::SimpleDijkstrasAlgorithm<Eigen::Vector3d>::PerformDijkstrasAlgorithm(
          free_space_graph_, free_space_graph_ind);
      dijkstras_results_[cover_ind] = result.second;

      //            std::cout << "Cover idx: " << cover_ind << " Dijktsras size: " <<
      //            dijkstras_results_[cover_ind].first.size() << std::endl;
...
const DijkstrasCoverageTask::Correspondences& DijkstrasCoverageTask::getCoverPointCorrespondences(
  ...
  if (current_correspondences_calculated_.load()) {
    ...
    if (current_correspondences_calculated_.load()) {
      ...
      ROS_INFO_STREAM_NAMED("task_specification",
                            "Calculated correspondences in        " << GlobalStopwatch(READ) << " seconds");
      ...
      if (visualize_correspondences_) {
        ...
        for (size_t deform_idx = 0; (ssize_t)deform_idx < num_nodes_; ++deform_idx) {
          ...
          for (size_t correspondence_idx = 0; correspondence_idx < current_deform_idx_correspondences.size();
               ++correspondence_idx) {
...
std::vector<EigenHelpers::VectorVector3d> DijkstrasCoverageTask::findPathFromObjectToTarget(
    const WorldState& world_state, const size_t max_steps) {
  ...
  for (size_t deformable_ind = 0; (ssize_t)deformable_ind < num_nodes_; ++deformable_ind) {
    dijkstras_paths[deformable_ind] = followCoverPointAssignments(
        object_configuration.col(deformable_ind), correspondences.correspondences_[deformable_ind], max_steps);
  }
...
ObjectDeltaAndWeight DijkstrasCoverageTask::calculateErrorCorrectionDeltaFixedCorrespondences(
    const WorldState& world_state, const std::vector<std::vector<ssize_t>>& correspondences) {
  ObjectDeltaAndWeight desired_object_delta(num_nodes_ * 3);
  ...
  for (ssize_t deform_idx = 0; deform_idx < num_nodes_; ++deform_idx) {
    ...
    const Eigen::Vector3d& deformable_point =
        sdf_->ProjectOutOfCollision3d(world_state.object_configuration_.col(deform_idx));
#else
    const Eigen::Vector3d& deformable_point = world_state.object_configuration_.col(deform_idx);
#endif
    const ssize_t deformable_point_idx_in_free_space_graph =
        work_space_grid_.worldPosToGridIndexClamped(deformable_point);
    ...
    for (size_t correspondence_idx = 0; correspondence_idx < current_correspondences.size(); ++correspondence_idx) {
      ...
      // Otherwise use the Dijkstra's result
        ...
        const ssize_t target_idx_in_free_space_graph =
            dijkstras_individual_result.first[deformable_point_idx_in_free_space_graph];
...
std::vector<double> DijkstrasCoverageTask::averageDijkstrasDistanceBetweenGrippersAndClusters(
    const Eigen::Isometry3d& gripper_pose, const std::vector<ssize_t>& cover_indices,
    const std::vector<uint32_t>& cluster_labels, const uint32_t num_clusters) const {
  assert(cover_indices.size() == cluster_labels.size());
  std::vector<std::vector<double>> distances(num_clusters);
  const ssize_t gripper_idx_in_free_space_graph =
      work_space_grid_.worldPosToGridIndexClamped(gripper_pose.translation());
  ...
  for (const auto& node : nodes) {
    ...
    for (const auto& edge : out_edges) {
      start_points.push_back(
          nodes[edge.GetFromIndex()].GetValueImmutable());  // Should be the same as start_points, but not verified
...
void DijkstrasCoverageTask::visualizeIndividualDijkstrasResult(const size_t cover_idx,
  ...
  for (ssize_t x_idx = 0; x_idx < work_space_grid_.getXNumSteps(); ++x_idx) {
    for (ssize_t y_idx = 0; y_idx < work_space_grid_.getYNumSteps(); ++y_idx) {
      for (ssize_t z_idx = 0; z_idx < work_space_grid_.getZNumSteps(); ++z_idx) {
        ...
        assert(point_idx_in_free_space_graph >= 0 &&
               point_idx_in_free_space_graph < (ssize_t)(dijkstras_next_target.size() - num_cover_points_));
        ...
        if (next_idx_in_free_space_graph >= 0) {
          const Eigen::Vector3d next_in_world =
              free_space_graph_.GetNodeImmutable(next_idx_in_free_space_graph).GetValueImmutable();
  ...
  vis_->visualizeArrows("dijkstras_navigation_field", start_points, end_points, Visualizer::Orange(),
                        work_space_grid_.minStepDimension() * 0.05, work_space_grid_.minStepDimension() * 0.1);
  vis_->forcePublishNow(0.2);
}
...
double DijkstrasCoverageTask::calculateError_impl(const WorldState& world_state) {
  const Correspondences& correspondences = getCoverPointCorrespondences(world_state);
...
ObjectDeltaAndWeight DijkstrasCoverageTask::calculateObjectErrorCorrectionDelta_impl(const WorldState& world_state) {
  const Correspondences& correspondences = getCoverPointCorrespondences(world_state);
  const ObjectPointSet& object_configuration = world_state.object_configuration_;
  ObjectDeltaAndWeight desired_object_delta(num_nodes_ * 3);
  ...
  for (ssize_t deform_idx = 0; deform_idx < num_nodes_; ++deform_idx) {
    ...
    const std::vector<bool>& current_correspondences_is_covered =
        correspondences.correspondences_is_covered_[deform_idx];
    const std::vector<double>& current_correspondences_distances =
        correspondences.correspondences_distances_[deform_idx];
    const EigenHelpers::VectorVector3d& current_correspondences_next_step =
        correspondences.correspondences_next_step_[deform_idx];

    for (size_t correspondence_idx = 0; correspondence_idx < current_correspondences.size(); ++correspondence_idx) {
      // If the current cover point is not covered (by this or another object point), then create a pull on the
      // cloth/rope
      if (!current_correspondences_is_covered[correspondence_idx]) {
        ...
        const double weight = std::max(desired_object_delta.weight(deform_idx * 3),
                                       current_correspondences_distances[correspondence_idx]);
        desired_object_delta.weight(deform_idx * 3) = weight;
        desired_object_delta.weight(deform_idx * 3 + 1) = weight;
        desired_object_delta.weight(deform_idx * 3 + 2) = weight;
      }
...
bool DijkstrasCoverageTask::saveDijkstrasResults() {
  try {
    ...
    const auto first_serializer = [](const std::vector<int64_t>& vec_to_serialize, std::vector<uint8_t>& buf) {
      return arc_utilities::SerializeVector<int64_t>(vec_to_serialize, buf,
                                                     &arc_utilities::SerializeFixedSizePOD<int64_t>);
    };
    const auto second_serializer = [](const std::vector<double>& vec_to_serialize, std::vector<uint8_t>& buf) {
      return arc_utilities::SerializeVector<double>(vec_to_serialize, buf,
                                                    &arc_utilities::SerializeFixedSizePOD<double>);
    };
    const auto pair_serializer = [&first_serializer, &second_serializer](
                                     const std::pair<std::vector<int64_t>, std::vector<double>>& pair_to_serialize,
                                     std::vector<uint8_t>& buf) {
      return arc_utilities::SerializePair<std::vector<int64_t>, std::vector<double>>(
          pair_to_serialize, buf, first_serializer, second_serializer);
    };
    arc_utilities::SerializeVector<std::pair<std::vector<int64_t>, std::vector<double>>>(dijkstras_results_, buffer,
                                                                                         pair_serializer);
...
bool DijkstrasCoverageTask::loadDijkstrasResults() {
  try {
    ...
    const uint64_t serialzed_graph_size =
        free_space_graph_.SerializeSelf(temp_buffer, &arc_utilities::SerializeEigen<double, 3, 1>);
    const auto mismatch_results =
        std::mismatch(temp_buffer.begin(), temp_buffer.end(), decompressed_dijkstras_results.begin());
    ...
    const auto first_deserializer = [](const std::vector<uint8_t>& buffer, const uint64_t current) {
      return arc_utilities::DeserializeVector<int64_t>(buffer, current,
                                                       &arc_utilities::DeserializeFixedSizePOD<int64_t>);
    };
    ...
    const auto pair_deserializer = [&first_deserializer, &second_deserializer](const std::vector<uint8_t>& buffer,
                                                                               const uint64_t current) {
      return arc_utilities::DeserializePair<std::vector<int64_t>, std::vector<double>>(
          buffer, current, first_deserializer, second_deserializer);
    };

    uint64_t current_position = serialzed_graph_size;
    const auto deserialized_result =
        arc_utilities::DeserializeVector<std::pair<std::vector<int64_t>, std::vector<double>>>(
            decompressed_dijkstras_results, current_position, pair_deserializer);
...
Eigen::Vector3d DijkstrasCoverageTask::sumVectorFields(const std::vector<ssize_t>& cover_point_assignments,
                                                       const Eigen::Vector3d& querry_loc) const {
  ...
  const Eigen::Vector3d& graph_aligned_querry_point =
      free_space_graph_.GetNodeImmutable(graph_aligned_querry_ind).GetValueImmutable();
  ...
  for (size_t assignment_ind = 0; assignment_ind < cover_point_assignments.size(); ++assignment_ind) {
    ...
    const Eigen::Vector3d& target_point =
        free_space_graph_.GetNodeImmutable(target_ind_in_work_space_graph).GetValueImmutable();
...
EigenHelpers::VectorVector3d DijkstrasCoverageTask::followCoverPointAssignments(
    const Eigen::Vector3d& starting_pos, const std::vector<ssize_t>& cover_point_assignments,
  ...
  for (size_t itr = 0; outer_progress && itr < maximum_iterations; ++itr) {
    ...
    // Split the delta up into smaller steps to simulate "pulling" the cloth along with constant obstacle collision
    // resolution
...
DistanceBasedCorrespondencesTask::DistanceBasedCorrespondencesTask(std::shared_ptr<ros::NodeHandle> nh,
                                                                   std::shared_ptr<ros::NodeHandle> ph,
                                                                   Visualizer::Ptr vis)
...
std::tuple<ssize_t, double, ssize_t, bool> DistanceBasedCorrespondencesTask::findNearestObjectPoint(
    const WorldState& world_state, const ssize_t cover_idx) const {
  const Eigen::Vector3d& cover_point = cover_points_.col(cover_idx);
  const auto& dijkstras_individual_result = dijkstras_results_[(size_t)cover_idx];
  ...
  for (ssize_t deformable_idx = 0; deformable_idx < num_nodes_; ++deformable_idx) {
    ...
    // First calculate the distance as defined by Dijkstras - code duplicated in
    // FixedCorrespondencesTask::calculateObjectErrorCorrectionDelta_impl
    double graph_dist;
    size_t target_idx_in_free_space_graph;
    {
      ...
      // Otherwise use the Dijkstra's result
        ...
        const ssize_t deformable_point_idx_in_free_space_graph =
            work_space_grid_.worldPosToGridIndexClamped(deformable_point);
        target_idx_in_free_space_graph =
            dijkstras_individual_result.first[(size_t)deformable_point_idx_in_free_space_graph];
        graph_dist = dijkstras_individual_result.second[(size_t)deformable_point_idx_in_free_space_graph];
      }
    }

    // Next, if we've found something closer than our record, update our record of the closest point on the deformable
    // object
    ...
    // Last, record if this point counts as covering the target (multiple deformable points may cover a single target
    // point)
    covered |= pointIsCovered(cover_idx, deformable_point);
  }
...
FixedCorrespondencesTask::FixedCorrespondencesTask(std::shared_ptr<ros::NodeHandle> nh,
                                                   std::shared_ptr<ros::NodeHandle> ph, Visualizer::Ptr vis)
    : DijkstrasCoverageTask(nh, ph, vis) {}
...
DijkstrasCoverageTask::Correspondences FixedCorrespondencesTask::getCoverPointCorrespondences_impl(
  ...
  for (size_t deform_idx = 0; (ssize_t)deform_idx < num_nodes_; ++deform_idx) {
    ...
    EigenHelpers::VectorVector3d& current_correspondences_next_step =
        correspondences_external.correspondences_next_step_[deform_idx];
    std::vector<double>& current_correspondences_distances =
        correspondences_external.correspondences_distances_[deform_idx];
    std::vector<bool>& current_correspondences_is_covered =
        correspondences_external.correspondences_is_covered_[deform_idx];
    ...
    for (size_t correspondence_idx = 0; correspondence_idx < current_correspondences.size(); ++correspondence_idx) {
      ...
      // Otherwise use the Dijkstra's result
        ...
        // Collect the Dijkstras next step and distance
        const auto& dijkstras_individual_result = dijkstras_results_[(size_t)cover_idx];
        const ssize_t target_ind_in_free_space_graph =
            dijkstras_individual_result.first[(size_t)nearest_idx_in_free_space_graph];
        ...
        //                std::cerr << "Target ind in free space graph: " << target_ind_in_free_space_graph <<
        //                std::endl; std::cerr << "Dijkstras_distance: " << dijkstras_distance << std::endl;

        current_correspondences_next_step.push_back(
            free_space_graph_.GetNodeImmutable(target_ind_in_free_space_graph).GetValueImmutable());
