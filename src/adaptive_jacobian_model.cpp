...
AdaptiveJacobianModel::AdaptiveJacobianModel(std::shared_ptr<ros::NodeHandle> nh, const MatrixXd& initial_jacobian,
                                             const double learning_rate)
    : JacobianModel(nh), current_jacobian_(initial_jacobian), learning_rate_(learning_rate) {
...
void AdaptiveJacobianModel::updateModel_impl(const WorldState& previous, const WorldState& next) {
  const AllGrippersSinglePoseDelta grippers_pose_deltas =
      CalculateGrippersPoseDelta(previous.all_grippers_single_pose_, next.all_grippers_single_pose_);

  const VectorXd grippers_delta = VectorEigenVectorToEigenVectorX(grippers_pose_deltas);

  if (grippers_delta.squaredNorm() < 1e-20) {
    ROS_WARN_STREAM_NAMED("adaptive_jacobian",
                          "Grippers did not move, not updating: squared norm vel: " << grippers_delta.squaredNorm());
  } else {
...
#pragma GCC diagnostic pop
    ...
    // Perform the update
    current_jacobian_ += learning_rate_ * (true_delta - predicted_delta) * grippers_delta.transpose() /
