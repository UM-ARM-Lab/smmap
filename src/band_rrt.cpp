...
static inline bool gripperPositionsAreApproximatelyEqual(const RRTGrippersRepresentation& c1,
...
static inline bool robotConfigurationsAreApproximatelyEqual(const RRTRobotRepresentation& r1,
                                                            const RRTRobotRepresentation& r2) {
  return r1.isApprox(r2);
}

static inline bool bandEndpointsMatchGripperPositions(const RubberBand& band,
...
static inline bool maxGrippersDistanceViolated(const Vector3d& gripper_a_pos, const Vector3d& gripper_b_pos,
                                               const double max_dist) {
  return (gripper_a_pos - gripper_b_pos).squaredNorm() > (max_dist * max_dist);
}

static inline bool maxGrippersDistanceViolated(const RRTGrippersRepresentation& grippers, const double max_dist) {
...
RRTNode::RRTNode() : initialized_(false) {}

RRTNode::RRTNode(const RRTGrippersRepresentation& grippers_poses, const RRTRobotRepresentation& robot_configuration,
...
RRTNode::RRTNode(const RRTGrippersRepresentation& grippers_poses, const RRTRobotRepresentation& robot_configuration,
                 const RubberBand::Ptr& band, const double cost_to_come, const double p_reachability,
                 const double p_transition, const int64_t parent_index, const int64_t state_index,
                 const int64_t transition_index, const int64_t split_index)
    : grippers_poses_(grippers_poses),
      robot_configuration_(robot_configuration),
      band_(band),
...
RRTNode::RRTNode(const RRTGrippersRepresentation& grippers_poses, const RRTRobotRepresentation& robot_configuration,
                 const QuinlanRubberBand::Ptr& band, const double cost_to_come, const double p_reachability,
                 const double p_transition, const double p_goal_reachable, const int64_t parent_index,
                 const int64_t state_index, const int64_t transition_index, const int64_t split_index,
                 const std::vector<int64_t>& child_indices, const bool initialized,
                 const bool already_extended_towards_backwards_tree, const bool blacklisted_from_nn_search)
    : grippers_poses_(grippers_poses),
      robot_configuration_(robot_configuration),
      band_(band),
      cost_to_come_(cost_to_come),
...
bool RRTNode::initialized() const { return initialized_; }

const RRTGrippersRepresentation& RRTNode::grippers() const { return grippers_poses_; }

const RRTRobotRepresentation& RRTNode::robotConfiguration() const { return robot_configuration_; }

const RubberBand::Ptr& RRTNode::band() const { return band_; }

double RRTNode::costToCome() const { return cost_to_come_; }

double RRTNode::pTransition() const { return p_transition_; }

double RRTNode::pReachability() const { return p_reachability_; }

double RRTNode::getpGoalReachable() const { return p_goal_reachable_; }
...
int64_t RRTNode::parentIndex() const { return parent_index_; }

int64_t RRTNode::stateIndex() const { return state_index_; }

int64_t RRTNode::transitionIndex() const { return transition_index_; }

int64_t RRTNode::splitIndex() const { return split_index_; }

const std::vector<int64_t>& RRTNode::childIndices() const { return child_indices_; }

void RRTNode::clearChildIndices() { child_indices_.clear(); }
...
uint64_t RRTNode::serialize(std::vector<uint8_t>& buffer) const {
  ...
  arc_utilities::SerializePair<Isometry3d, Isometry3d>(
      grippers_poses_, buffer, &arc_utilities::SerializeEigen<Isometry3d>, &arc_utilities::SerializeEigen<Isometry3d>);
...
uint64_t RRTNode::Serialize(const RRTNode& config, std::vector<uint8_t>& buffer) { return config.serialize(buffer); }

std::pair<RRTNode, uint64_t> RRTNode::Deserialize(const std::vector<uint8_t>& buffer, const uint64_t current,
  ...
  const auto grippers_poses_deserialized = arc_utilities::DeserializePair<Isometry3d, Isometry3d>(
      buffer, current_position, &arc_utilities::DeserializeEigen<Isometry3d>,
      &arc_utilities::DeserializeEigen<Isometry3d>);
  ...
  const auto child_indices_deserialized = arc_utilities::DeserializeVector<int64_t>(
      buffer, current_position, &arc_utilities::DeserializeFixedSizePOD<int64_t>);
  ...
  const auto already_extended_towards_goal_set_deserialized =
  ...
  const auto blacklisted_from_nn_search_deserialized =
      arc_utilities::DeserializeFixedSizePOD<uint8_t>(buffer, current_position);
  current_position += blacklisted_from_nn_search_deserialized.second;

  // Build the resulting node
  RRTNode deserialized(
      grippers_poses_deserialized.first, robot_configuration_deserialized.first, band, cost_to_come_deserialized.first,
      p_reachablity_deserialized.first, p_transition_deserialized.first, p_goal_reachable_deserialized.first,
      parent_index_deserialized.first, state_index_deserialized.first, transition_index_deserialized.first,
      split_index_deserialized.first, child_indices_deserialized.first, (bool)initialized_deserialized.first,
      (bool)already_extended_towards_goal_set_deserialized.first, (bool)blacklisted_from_nn_search_deserialized.first);

  const uint64_t bytes_read = current_position - current;
  return std::make_pair(deserialized, bytes_read);
}

std::ostream& smmap::operator<<(std::ostream& out, const RRTNode& node) { return out << node.print(); }

////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
///////////////           RRTDistance functions                    /////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

const RRTRobotRepresentation& RRTDistance::GetJointWeights() { return joint_weights_; }

void RRTDistance::SetJointWeights(const RRTRobotRepresentation joint_weights) { joint_weights_ = joint_weights; }
...
double RRTDistance::DistanceSquared(const RRTGrippersRepresentation& c1, const RRTGrippersRepresentation& c2) {
  ...
  const auto& c2_first_gripper = c2.first.translation();
  const auto& c2_second_gripper = c2.second.translation();
  return (c1_first_gripper - c2_first_gripper).squaredNorm() + (c1_second_gripper - c2_second_gripper).squaredNorm();
}
...
BandRRT::BandRRT(std::shared_ptr<ros::NodeHandle> nh, std::shared_ptr<ros::NodeHandle> ph,
                 const WorldParams& world_params, const PlanningParams& planning_params,
                 const SmoothingParams& smoothing_params, const TaskParams& task_params,
                 const RubberBand::ConstPtr& template_band, Visualizer::Ptr vis, const bool visualization_enabled)
    : nh_(nh),
      ...
      band_max_dist2_(band_distance2_scaling_factor_ * (double)band_max_points_ *
                      (task_aligned_upper_limits_ - task_aligned_lower_limits_).squaredNorm())
 ...
 */
RRTPolicy BandRRT::plan(const RRTNode& start, const RRTGrippersRepresentation& grippers_goal_poses,
  ...
  forward_tree_.push_back(RRTNode(starting_grippers_poses_, starting_robot_configuration_, starting_band_, cost_to_come,
                                  p_reachability, p_transition, parent_index, state_index, transition_index,
                                  split_index));
  ...
  if (planning_for_whole_robot_) {
    const auto goal_configurations =
        robot_->getCloseIkSolutions({grippers_goal_poses.first, grippers_goal_poses.second}, max_grippers_distance_);
    ...
    for (size_t idx = 0; idx < goal_configurations.size(); ++idx) {
      grippers_goal_set_.push_back(RRTNode(grippers_goal_poses_, goal_configurations[idx], start.band()));
    }
  } else {
  ...
  if (first_gripper_dist_to_env < gripper_min_distance_to_obstacles_ ||
    ...
    const double dist_between_grippers =
        (grippers_goal_poses_.first.translation() - grippers_goal_poses_.second.translation()).norm();
    std::cerr << "Unfeasible goal location: " << grippers_goal_poses_.first.translation().transpose() << "  :  "
              << grippers_goal_poses_.second.translation().transpose() << std::endl;
    std::cerr << "Min gripper collision distance: " << gripper_min_distance_to_obstacles_
              << " Current Distances: " << first_gripper_dist_to_env << " " << second_gripper_dist_to_env << std::endl;
    std::cerr << "Max allowable distance: " << max_grippers_distance_
              << " Distance beteween goal grippers: " << dist_between_grippers << std::endl;

    vis_->visualizeGrippers("weird_gripper_goals",
                            VectorIsometry3d{grippers_goal_poses_.first, grippers_goal_poses_.second},
                            Visualizer::Red(), 1);
  ...
  forward_nn_index_ = std::make_shared<NNIndexType>(flann::KDTreeSingleIndexParams(),
                                                    flann::L2_weighted<float>(robot_->getJointWeights()));
  forward_next_idx_to_add_to_nn_dataset_ = 0;
  ...
  goal_set_nn_index_ = std::make_shared<NNIndexType>(flann::KDTreeSingleIndexParams(),
                                                     flann::L2_weighted<float>(robot_->getJointWeights()));
  goal_set_next_idx_to_add_to_nn_dataset_ = 0;

  // Force an initializion the backward tree flann data
  //    const bool force_rebuild = true;
  //    rebuildNNIndex(goal_set_nn_index_, goal_set_nn_raw_data_, goal_set_nn_data_idx_to_tree_idx_, grippers_goal_set_,
  //    goal_set_next_idx_to_add_to_nn_dataset_, force_rebuild);
  ...
  if (useStoredTree()) {
    ...
    ROS_INFO_STREAM_NAMED("rrt", "Finished planning, for better or worse. Path found with probability "
                                     << forward_tree_[0].getpGoalReachable());

    planning_statistics_["planning_time0_sampling                                 "] = total_sampling_time_;
    planning_statistics_["planning_time1_1_nearest_neighbour_index_building       "] =
        total_nearest_neighbour_index_building_time_;
    planning_statistics_["planning_time1_2_nearest_neighbour_index_searching      "] =
        total_nearest_neighbour_index_searching_time_;
    planning_statistics_["planning_time1_3_nearest_neighbour_linear_searching     "] =
        total_nearest_neighbour_linear_searching_time_;
    planning_statistics_["planning_time1_4_nearest_neighbour_radius_searching     "] =
        total_nearest_neighbour_radius_searching_time_;
    planning_statistics_["planning_time1_5_nearest_neighbour_best_searching       "] =
        total_nearest_neighbour_best_searching_time_;
    ...
    planning_statistics_["planning_time2_4_forward_propogation_band               "] =
        total_band_forward_propogation_time_ - transition_estimator_->classifierTime();
    planning_statistics_["planning_time2_5_classifier                             "] =
        transition_estimator_->classifierTime();
    planning_statistics_["planning_time2_6_forward_propogation_first_order_vis    "] =
        total_first_order_vis_propogation_time_;
    planning_statistics_["planning_time2_forward_propogation_everything_included  "] =
        total_everything_included_forward_propogation_time_;
    planning_statistics_["planning_time3_total                                    "] = planning_time.count();

    planning_statistics_["planning_size00_forward_random_samples_useless          "] =
        (double)forward_random_samples_useless_;
    planning_statistics_["planning_size01_forward_random_samples_useful           "] =
        (double)forward_random_samples_useful_;
    planning_statistics_["planning_size02_forward_states                          "] = (double)forward_tree_.size();

    planning_statistics_["planning_size03_backward_random_samples_useless         "] =
        (double)backward_random_samples_useless_;
    planning_statistics_["planning_size04_backward_random_samples_useful          "] =
        (double)backward_random_samples_useful_;
    planning_statistics_["planning_size05_backward_states                         "] =
        (double)grippers_goal_set_.size();

    planning_statistics_["planning_size06_forward_connection_attempts_useless     "] =
        (double)forward_connection_attempts_useless_;
    planning_statistics_["planning_size07_forward_connection_attempts_useful      "] =
        (double)forward_connection_attempts_useful_;
    planning_statistics_["planning_size08_forward_connections_made                "] =
        (double)forward_connections_made_;

    planning_statistics_["planning_classifier_num0_band_wierdness                 "] =
        (double)transition_estimator_->numBandWeirdness();
    planning_statistics_["planning_classifier_num1_band_safe                      "] =
        (double)transition_estimator_->numBandSafe();
    planning_statistics_["planning_classifier_num2_band_overstretch               "] =
        (double)transition_estimator_->numBandOverstretch();
    planning_statistics_["planning_classifier_num3_band_no_mistake                "] =
        (double)transition_estimator_->numNoMistakes();
    planning_statistics_["planning_classifier_num4_band_mistakes                  "] =
        (double)transition_estimator_->numMistakes();
    planning_statistics_["planning_classifier_num5_band_accepted_mistakes         "] =
        (double)transition_estimator_->numAcceptedMistakes();

    //        planning_statistics_["planning_size09_backward_connection_attempts_useless    "] =
    //        (double)backward_connection_attempts_useless_;
    //        planning_statistics_["planning_size10_backward_connection_attempts_useful     "] =
    //        (double)backward_connection_attempts_useful_;
    //        planning_statistics_["planning_size11_backward_connections_made               "] =
    //        (double)backward_connections_made_;
...
void BandRRT::clearBlacklist() { blacklisted_goal_rubber_bands_.clear(); }
    ...
    if ((parent_index >= 0) && (parent_index < (int64_t)tree.size())) {
      if (parent_index != (int64_t)current_index) {
        ...
        if (index_found == parent_child_indices.end()) {
          std::cerr << "Parent state " << parent_index << " does not contain child index for current node "
                    << current_index << std::endl;
          return false;
        }
      } else {
        std::cerr << "Invalid parent index " << parent_index << " for state " << current_index
      ...
      if ((current_child_index > 0) && (current_child_index < (int64_t)tree.size())) {
        if (current_child_index != (int64_t)current_index) {
          ...
          if (child_parent_index != (int64_t)current_index) {
            std::cerr << "Parent index " << child_parent_index << " for current child state " << current_child_index
                      << " does not match index " << current_index << " for current node " << std::endl;
            return false;
          }
        } else {
          std::cerr << "Invalid child index " << current_child_index << " for state " << current_index
                    << " [Indices can't be the same]" << std::endl;
          return false;
        }
      } else {
...
bool BandRRT::CheckPathLinkage(const RRTPath& path) {
  ...
  for (size_t current_index = 0; current_index < path.size(); current_index++) {
    ...
    if (current_child_indices[0] != (int64_t)current_index + 1) {
      std::cerr << "Path contains a node whose children are not the next node in the path " << current_index
                << std::endl;
      return false;
    }
  }
...
RRTPolicy BandRRT::ExtractSolutionPolicy(const RRTTree& tree) {
  ...
  if (tree.empty()) {
    ROS_ERROR_NAMED("rrt",
                    "Asked to extract policy from empty tree; this doesn't make sense. Returning an empty policy");
    return {};
  }
  ...
  while (!path_roots.empty()) {
    ...
    path_roots.pop();
    ++next_path_idx;
    ROS_INFO_STREAM_COND_NAMED(SMMAP_RRT_VERBOSE, "rrt",
                               "Popped node with state idx: " << path_root.stateIndex() << " from path roots queue");
    ...
    do {
      ...
      ROS_INFO_STREAM_COND_NAMED(SMMAP_RRT_VERBOSE, "rrt",
                                 "    Added node with state idx: " << curr_node.stateIndex() << " to path");
      ...
      ROS_INFO_STREAM_COND_NAMED(SMMAP_RRT_VERBOSE, "rrt",
                                 "    " << effective_child_branches.size() << " transitions taken out of this node");

      // Now that we have the splits separated out, compute the goal probability of each transition, keeping only the
      // largest
      double p_goal_reachable = 0.0;
      std::vector<int64_t> best_transition_idx;  // should be only one element long, ideally. A vector is stored to
                                                 // catch edge conditions that we have not yet decided how to resolve.
      for (auto itr = effective_child_branches.begin(); itr != effective_child_branches.end(); ++itr) {
        ...
        if (p_goal_reachable_current_transition > 0.0 && p_goal_reachable_current_transition > p_goal_reachable) {
        ...
        ROS_INFO_STREAM_COND_NAMED(SMMAP_RRT_VERBOSE, "rrt",
                                   "    Transition idx " << itr->first << " p_goal_reachable_current_transition: "
                                                         << p_goal_reachable_current_transition);
      }
      ...
      if (best_transition_idx.size() == 0) {
        ...
        std::cerr << "Unhandled edge case when considering 'best transition'.\n"
                  << "    p_goal_reachable = " << p_goal_reachable
                  << "    Transitions: " << PrettyPrint::PrettyPrint(best_transition_idx, false, " ") << std::endl;
...
std::vector<Visualizer::NamespaceId> BandRRT::visualizeTree(
    const RRTTree& tree, const size_t start_idx, const std::string ns_a, const std::string ns_b,
    const std::string ns_band, const int id_a, const int id_b, const int id_band, const std_msgs::ColorRGBA& color_a,
    const std_msgs::ColorRGBA& color_b, const std_msgs::ColorRGBA& color_band, const bool draw_band) const {
  ...
  if (visualization_enabled_globally_) {
    ...
    for (size_t idx = start_idx; idx < tree.size(); ++idx) {
      ...
      if (!curr.initialized() && tree == forward_tree_) {
        std::cout << "Node idx: " << idx << std::endl << curr.print() << std::endl;
      }
...
std::vector<Visualizer::NamespaceId> BandRRT::visualizeBothTrees() const {
  ...
  const auto forward_tree_ids =
      visualizeTree(forward_tree_, 0, RRT_FORWARD_TREE_GRIPPER_A_NS, RRT_FORWARD_TREE_GRIPPER_B_NS, RRT_TREE_BAND_NS, 1,
                    1, 1, gripper_a_forward_tree_color_, gripper_b_forward_tree_color_, band_tree_color_, draw_band);
  marker_ids.insert(marker_ids.end(), forward_tree_ids.begin(), forward_tree_ids.end());

  const auto backward_tree_ids = visualizeTree(
      grippers_goal_set_, 0, RRT_BACKWARD_TREE_GRIPPER_A_NS, RRT_BACKWARD_TREE_GRIPPER_B_NS, RRT_TREE_BAND_NS, 2, 2, 2,
      gripper_a_backward_tree_color_, gripper_b_backward_tree_color_, band_tree_color_, draw_band);
...
void BandRRT::deleteTreeVisualizations() const {
  ...
  vis_->visualizeCubes(TaskFramework::CLUSTERING_RESULTS_POST_PROJECT_NS, {grippers_goal_poses_.first.translation()},
                       Vector3d::Ones() * work_space_grid_.minStepDimension(), gripper_a_forward_tree_color_, 1);
  vis_->visualizeCubes(TaskFramework::CLUSTERING_RESULTS_POST_PROJECT_NS, {grippers_goal_poses_.second.translation()},
                       Vector3d::Ones() * work_space_grid_.minStepDimension(), gripper_b_forward_tree_color_, 5);
}

// Assumes the path that is passed is sequential
std::vector<Visualizer::NamespaceId> BandRRT::visualizePath(const RRTPath& path, const std::string& ns_prefix,
                                                            const int32_t id, const bool draw_band) const {
  std::vector<Visualizer::NamespaceId> marker_ids;
  ...
  const auto gripper_a_ids = vis_->visualizeCubes(ns_prefix + RRT_PATH_GRIPPER_A_NS, gripper_a_cubes,
                                                  Vector3d(0.005, 0.005, 0.005), gripper_a_forward_tree_color_, id);
  marker_ids.insert(marker_ids.end(), gripper_a_ids.begin(), gripper_a_ids.end());
  const auto gripper_b_ids = vis_->visualizeCubes(ns_prefix + RRT_PATH_GRIPPER_B_NS, gripper_b_cubes,
                                                  Vector3d(0.005, 0.005, 0.005), gripper_b_forward_tree_color_, id);
  marker_ids.insert(marker_ids.end(), gripper_b_ids.begin(), gripper_b_ids.end());
  if (draw_band) {
    const auto band_ids = vis_->visualizeLines(ns_prefix + RRT_PATH_RUBBER_BAND_NS, line_start_points, line_end_points,
                                               Visualizer::Blue(), id);
    marker_ids.insert(marker_ids.end(), band_ids.begin(), band_ids.end());
  }
...
std::vector<Visualizer::NamespaceId> BandRRT::visualizeBlacklist() const {
  ...
  for (size_t idx = 0; idx < blacklisted_goal_rubber_bands_.size(); ++idx) {
    ...
    const auto new_ids =
        band->visualize(RRT_BLACKLISTED_GOAL_BANDS_NS, Visualizer::Red(), Visualizer::Red(), (int32_t)(idx + 1));
  ...
  try {
    if (file_path.empty()) {
      ...
      const auto file_name_prefix =
          ROSHelpers::GetParamRequiredDebugLog<std::string>(*ph_, "path_file_name_prefix", __func__);
...
RRTTree BandRRT::loadStoredTree(std::string file_path) const {
  try {
    if (file_path.empty()) {
      ...
      const auto file_name_suffix =
          ROSHelpers::GetParamRequiredDebugLog<std::string>(*ph_, "path_file_name_suffix_to_load", __func__);
      if (!file_name_suffix.Valid()) {
        throw_arc_exception(std::invalid_argument,
                            "Unable to load path_file_name_suffix_to_load from parameter server");
      }

      const std::string file_name =
          file_name_prefix.GetImmutable() + "__" + file_name_suffix.GetImmutable() + ".compressed";
      file_path = log_folder.GetImmutable() + file_name;
    }
...
void BandRRT::planningMainLoop() {
  ...
  while (!path_found && time_ellapsed < time_limit_) {
    ROS_INFO_STREAM_COND_NAMED(SMMAP_RRT_VERBOSE, "rrt",
                               "Starting forward iteration. Tree size: " << forward_tree_.size());
    ...
    if (forward_tree_[0].getpGoalReachable() == 1.0) {
      ...
      const int64_t last_node_idx_in_forward_tree_branch =
          num_random_nodes_created > 0 ? (int64_t)forward_tree_.size() - 1 : forward_tree_start_idx;
      ...
      if (!last_node.already_extended_towards_goal_set_ && sample_goal) {
    ...
    ROS_INFO_STREAM_COND_NAMED(SMMAP_RRT_VERBOSE, "rrt",
                               "Ending forward iteration. Tree size: " << forward_tree_.size());
    ...
    if (main_loop_itr % 10000 == 0) {
      ...
      partial_stats["planning_partial_time1_1_nearest_neighbour_index_building       "] =
          total_nearest_neighbour_index_building_time_;
      partial_stats["planning_partial_time1_2_nearest_neighbour_index_searching      "] =
          total_nearest_neighbour_index_searching_time_;
      partial_stats["planning_partial_time1_3_nearest_neighbour_linear_searching     "] =
          total_nearest_neighbour_linear_searching_time_;
      partial_stats["planning_partial_time1_4_nearest_neighbour_radius_searching     "] =
          total_nearest_neighbour_radius_searching_time_;
      partial_stats["planning_partial_time1_5_nearest_neighbour_best_searching       "] =
          total_nearest_neighbour_best_searching_time_;
      partial_stats["planning_partial_time1_nearest_neighbour                        "] = total_nearest_neighbour_time_;
      partial_stats["planning_partial_time2_1_forward_propogation_fk                 "] =
          total_forward_kinematics_time_;
      ...
      partial_stats["planning_partial_time2_4_forward_propogation_band               "] =
          total_band_forward_propogation_time_ - transition_estimator_->classifierTime();
      partial_stats["planning_partial_time2_5_classifier                             "] =
          transition_estimator_->classifierTime();
      partial_stats["planning_partial_time2_6_forward_propogation_first_order_vis    "] =
          total_first_order_vis_propogation_time_;
      partial_stats["planning_partial_time2_forward_propogation_everything_included  "] =
          total_everything_included_forward_propogation_time_;
      partial_stats["planning_partial_time3_total                                    "] = planning_time.count();

      partial_stats["planning_partial_size00_forward_random_samples_useless          "] =
          (double)forward_random_samples_useless_;
      partial_stats["planning_partial_size01_forward_random_samples_useful           "] =
          (double)forward_random_samples_useful_;
      partial_stats["planning_partial_size02_forward_states                          "] = (double)forward_tree_.size();

      partial_stats["planning_partial_size03_backward_random_samples_useless         "] =
          (double)backward_random_samples_useless_;
      partial_stats["planning_partial_size04_backward_random_samples_useful          "] =
          (double)backward_random_samples_useful_;
      partial_stats["planning_partial_size05_backward_states                         "] =
          (double)grippers_goal_set_.size();

      partial_stats["planning_partial_size06_forward_connection_attempts_useless     "] =
          (double)forward_connection_attempts_useless_;
      partial_stats["planning_partial_size07_forward_connection_attempts_useful      "] =
          (double)forward_connection_attempts_useful_;
      partial_stats["planning_partial_size08_forward_connections_made                "] =
          (double)forward_connections_made_;

      partial_stats["planning_partial_classifier_num0_band_wierdness                 "] =
          (double)transition_estimator_->numBandWeirdness();
      partial_stats["planning_partial_classifier_num1_band_safe                      "] =
          (double)transition_estimator_->numBandSafe();
      partial_stats["planning_partial_classifier_num2_band_overstretch               "] =
          (double)transition_estimator_->numBandOverstretch();
      partial_stats["planning_partial_classifier_num3_band_no_mistake                "] =
          (double)transition_estimator_->numNoMistakes();
      partial_stats["planning_partial_classifier_num4_band_mistakes                  "] =
          (double)transition_estimator_->numMistakes();
      partial_stats["planning_partial_classifier_num5_band_accepted_mistakes         "] =
          (double)transition_estimator_->numAcceptedMistakes();

      //        partial_stats["planning_partial_size09_backward_connection_attempts_useless    "] =
      //        (double)backward_connection_attempts_useless_;
      //        partial_stats["planning_partial_size10_backward_connection_attempts_useful     "] =
      //        (double)backward_connection_attempts_useful_;
      //        partial_stats["planning_partial_size11_backward_connections_made               "] =
      //        (double)backward_connections_made_;

      ROS_INFO_STREAM_NAMED("rrt", "Planning Statistics @ Main Loop Itr: "
                                       << main_loop_itr << "\n"
      ...
      if (forward_tree_next_visualized_node_ < forward_tree_.size()) {
        ...
        visualizeTree(forward_tree_, forward_tree_next_visualized_node_, RRT_FORWARD_TREE_GRIPPER_A_NS,
                      RRT_FORWARD_TREE_GRIPPER_B_NS, RRT_TREE_BAND_NS, tree_marker_id_, tree_marker_id_, 1,
                      gripper_a_forward_tree_color_, gripper_b_forward_tree_color_, band_tree_color_, draw_band);
        ++tree_marker_id_;
...
RRTNode BandRRT::configSampling(const bool sample_band) {
  ...
  if (!planning_for_whole_robot_) {
    ...
    sample = RRTNode(gripper_poses, robot_config, std::make_shared<RubberBand>(template_band_));
  } else {
    sample = RRTNode(starting_grippers_poses_, robotConfigPairSampling_internal(),
                     std::make_shared<RubberBand>(template_band_));
  }

  ROS_INFO_STREAM_COND_NAMED(SMMAP_RRT_VERBOSE, "rrt.sampling",
                             "Random robot config: " << sample.robotConfiguration().transpose());
...
RRTGrippersRepresentation BandRRT::posPairSampling_internal() {
  ...
  do {
    const double x1 = Interpolate(task_aligned_lower_limits_.x(), task_aligned_upper_limits_.x(),
                                  uniform_unit_distribution_(*generator_));
    const double y1 = Interpolate(task_aligned_lower_limits_.y(), task_aligned_upper_limits_.y(),
                                  uniform_unit_distribution_(*generator_));
    const double z1 = Interpolate(task_aligned_lower_limits_.z(), task_aligned_upper_limits_.z(),
                                  uniform_unit_distribution_(*generator_));
    gripper_a_pos = Vector3d(x1, y1, z1);
  } while (sdf_->EstimateDistance3d(task_aligned_frame_transform_ * gripper_a_pos).first <
           gripper_min_distance_to_obstacles_);
  ...
  } while (!valid || sdf_->EstimateDistance3d(task_aligned_frame_transform_ * gripper_b_pos).first <
                         gripper_min_distance_to_obstacles_);
...
RRTRobotRepresentation BandRRT::robotConfigPairSampling_internal() {
  ...
  for (ssize_t idx = 0; idx < total_dof_; ++idx) {
    rand_sample(idx) = Interpolate(robot_joint_lower_limits_(idx), robot_joint_upper_limits_(idx),
                                   uniform_unit_distribution_(*generator_));
  }
  return rand_sample;
}
...
VectorVector3d BandRRT::bandSampling_internal() {
  ...
  for (size_t idx = 0; idx < band_max_points_; ++idx) {
    const double x = Interpolate(task_aligned_lower_limits_.x(), task_aligned_upper_limits_.x(),
                                 uniform_unit_distribution_(*generator_));
    const double y = Interpolate(task_aligned_lower_limits_.y(), task_aligned_upper_limits_.y(),
                                 uniform_unit_distribution_(*generator_));
    const double z = Interpolate(task_aligned_lower_limits_.z(), task_aligned_upper_limits_.z(),
                                 uniform_unit_distribution_(*generator_));
...
static std::pair<int64_t, double> getNearest(const RRTRobotRepresentation& robot_config, const NNIndexType& index,
...
static std::pair<int64_t, double> getNearest(const RRTRobotRepresentation& robot_config, const RRTTree& tree,
...
static std::vector<std::pair<int64_t, double>> radiusSearch(const RRTRobotRepresentation& robot_config,
...
static std::vector<std::pair<int64_t, double>> radiusSearch(const RRTRobotRepresentation& robot_config,
                                                            const RRTTree& tree, const size_t start_idx,
                                                            const double radius2) {
  std::vector<std::pair<int64_t, double>> near;
...
static std::pair<int64_t, double> getNearestFullConfig(
    const RRTNode& config, const RRTTree& tree, const double band_distance2_scaling_factor_,
...
static std::pair<int64_t, double> getBestFullConfig(
    const RRTTree& tree, const std::vector<std::pair<int64_t, double>>& radius_search_set_1,
    const std::vector<std::pair<int64_t, double>>& radius_search_set_2) {
  assert(radius_search_set_1.size() + radius_search_set_2.size() > 0);
...
int64_t BandRRT::nearestNeighbour(const bool use_forward_tree, const RRTNode& config) {
  Stopwatch stopwatch;
...
std::pair<int64_t, double> BandRRT::nearestNeighbourRobotSpace(const bool use_forward_tree, const RRTNode& config) {
  ...
  if (use_forward_tree) {
    ...
    nn_data_idx_to_tree_idx = &forward_nn_data_idx_to_tree_idx_, nn_raw_data = &forward_nn_raw_data_;
    ...
    nn_data_idx_to_tree_idx = &goal_set_nn_data_idx_to_tree_idx_, nn_raw_data = &goal_set_nn_raw_data_;
    manual_search_start_idx = &goal_set_next_idx_to_add_to_nn_dataset_;
  }

  //    ROS_INFO_STREAM_COND_NAMED(SMMAP_RRT_VERBOSE, "rrt.nn", "Querry:                    " <<
  //    config.robotConfiguration().transpose());

  // Check if we should rebuild the NN Index
  if (!use_brute_force_nn_ && *manual_search_start_idx + kd_tree_grow_threshold_ <= tree->size()) {
    ROS_INFO_STREAM_COND_NAMED(SMMAP_RRT_VERBOSE, "rrt.nn",
                               "Rebuilding FLANN index; forward tree? " << use_forward_tree);
    ROS_INFO_STREAM_COND_NAMED(SMMAP_RRT_VERBOSE, "rrt.nn", "RRT tree size: " << tree->size());
    ROS_INFO_STREAM_COND_NAMED(SMMAP_RRT_VERBOSE, "rrt.nn",
                               "Initial manual search start idx: " << manual_search_start_idx);
    ...
    ROS_INFO_STREAM_COND_NAMED(SMMAP_RRT_VERBOSE, "rrt.nn",
                               "Final manual search start idx: " << manual_search_start_idx);
  ...
  if (*manual_search_start_idx < tree->size()) {
    ...
    const std::pair<int64_t, double> linear_nearest =
        getNearest(config.robotConfiguration(), *tree, *manual_search_start_idx);
...
int64_t BandRRT::nearestBestNeighbourFullSpace(const RRTNode& config) {
  ...
  if (!use_brute_force_nn_ && forward_next_idx_to_add_to_nn_dataset_ > 0) {
    flann_radius_result = radiusSearch(config.robotConfiguration(), *forward_nn_index_,
                                       forward_nn_data_idx_to_tree_idx_, nearest_robot_space.second + band_max_dist2_);
  }

  // If we have data that isn't in the FLANN index
  std::vector<std::pair<int64_t, double>> linear_radius_result;
  if (forward_next_idx_to_add_to_nn_dataset_ < forward_tree_.size()) {
    linear_radius_result =
        radiusSearch(config.robotConfiguration(), forward_tree_, forward_next_idx_to_add_to_nn_dataset_,
  ...
  const std::pair<int64_t, double> nearest_full_space = getNearestFullConfig(
      config, forward_tree_, band_distance2_scaling_factor_, flann_radius_result, linear_radius_result);
  ...
  if (nearest_full_space.second <= best_near_radius2_ + band_max_dist2_) {
    ...
    if (nearest_robot_space.second < best_near_radius2_) {
      if (!use_brute_force_nn_ && forward_next_idx_to_add_to_nn_dataset_ > 0) {
        flann_best_near_radius_result = radiusSearch(config.robotConfiguration(), *forward_nn_index_,
                                                     forward_nn_data_idx_to_tree_idx_, best_near_radius2_);
      }
      if (forward_next_idx_to_add_to_nn_dataset_ < forward_tree_.size()) {
        linear_best_near_radius_result = radiusSearch(config.robotConfiguration(), forward_tree_,
                                                      forward_next_idx_to_add_to_nn_dataset_, best_near_radius2_);
      }
    }

    const std::pair<int64_t, double> best_full_space =
        getBestFullConfig(forward_tree_, flann_best_near_radius_result, linear_best_near_radius_result);
...
void BandRRT::rebuildNNIndex(std::shared_ptr<NNIndexType> index, std::vector<float>& nn_raw_data,
                             std::vector<size_t>& nn_data_idx_to_tree_idx, const RRTTree& tree,
                             size_t& new_data_start_idx, const bool force_rebuild) {
  if (force_rebuild) {
...
size_t BandRRT::connectForwardTree(const int64_t forward_tree_start_idx, const RRTNode& target, const bool is_random) {
  ...
  const size_t num_random_nodes_created = forwardPropogationFunction(
      forward_tree_, forward_tree_start_idx, target, allow_mistakes, fwd_prop_local_visualization_enabled);

  ROS_INFO_STREAM_COND_NAMED(SMMAP_RRT_VERBOSE, "rrt",
                             "Nodes created: " << num_random_nodes_created << " Tree size: " << forward_tree_.size());
  ...
  if (SMMAP_RRT_VERBOSE) {
    for (size_t idx = forward_tree_.size() - num_random_nodes_created; idx < forward_tree_.size(); ++idx) {
      ...
      ROS_INFO_STREAM_NAMED("rrt", "Node idx: " << idx << " Parent: " << node.parentIndex()
                                                << " Config: " << node.robotConfiguration().transpose());
    }
  }
...
size_t BandRRT::connectTreeToGrippersGoalSet(const int64_t last_node_idx_in_forward_tree_branch) {
  ...
  const int64_t backward_tree_nearest_neighbour_idx =
      nearestNeighbour(nn_forward_tree, forward_tree_[last_node_idx_in_forward_tree_branch]);
  const RRTNode& target_in_backward_tree = grippers_goal_set_[backward_tree_nearest_neighbour_idx];

  const size_t num_goal_directed_nodes_created =
      forwardPropogationFunction(forward_tree_, last_node_idx_in_forward_tree_branch, target_in_backward_tree,
                                 allow_mistakes, fwd_prop_local_visualization_enabled);
...
size_t BandRRT::forwardPropogationFunction(RRTTree& tree_to_extend, const int64_t& nearest_neighbor_idx,
                                           const RRTNode& target, const bool allow_mistakes,
  ...
  if (false && visualization_enabled_globally_ && visualization_enabled_locally) {
    vis_->visualizeCubes(RRT_FORWARD_PROP_START_NS, {nearest_neighbour.grippers().first.translation()},
                         Vector3d(0.01, 0.01, 0.01), gripper_a_forward_tree_color_, 1);
    vis_->visualizeCubes(RRT_FORWARD_PROP_START_NS, {nearest_neighbour.grippers().second.translation()},
                         Vector3d(0.01, 0.01, 0.01), gripper_b_forward_tree_color_, 5);

    nearest_neighbour.band()->visualize(RRT_FORWARD_PROP_START_NS, Visualizer::Green(), Visualizer::Green(), 10, true);

    vis_->visualizeCubes(RRT_SAMPLE_NS, {target.grippers().first.translation()}, Vector3d(0.01, 0.01, 0.01),
                         gripper_a_forward_tree_color_, 1);
    vis_->visualizeCubes(RRT_SAMPLE_NS, {target.grippers().second.translation()}, Vector3d(0.01, 0.01, 0.01),
                         gripper_b_forward_tree_color_, 5);
  }
  ...
  while (!split_happened && step_index < max_total_steps) {
    ...
    if (planning_for_whole_robot_) {
      ...
      {// Check if we rotated the grippers too much
       {const double gripper_a_rotation_dist =
            EigenHelpers::Distance(starting_grippers_poses_.first.rotation(), next_grippers_poses.first.rotation());
      const double gripper_b_rotation_dist =
          EigenHelpers::Distance(starting_grippers_poses_.second.rotation(), next_grippers_poses.second.rotation());
  ...
  // Collision checking
  {
    stopwatch(RESET);
    arc_helpers::DoNotOptimize(next_grippers_poses);
    const bool in_collision_sdf = (sdf_->EstimateDistance3d(next_grippers_poses.first.translation()).first <
                                   gripper_min_distance_to_obstacles_) ||
                                  (sdf_->EstimateDistance3d(next_grippers_poses.second.translation()).first <
                                   gripper_min_distance_to_obstacles_) ||
                                  (sdf_->DistanceToBoundary3d(next_grippers_poses.first.translation()).first <
                                   gripper_min_distance_to_obstacles_) ||
                                  (sdf_->DistanceToBoundary3d(next_grippers_poses.second.translation()).first <
                                   gripper_min_distance_to_obstacles_);
  ...
  // Collision checking
  {
    // If the grippers collide with each other, then return however far we are able to get
    {
      ...
      const bool in_collision =
          (next_grippers_poses.first.translation() - next_grippers_poses.second.translation()).norm() <
          gripper_min_distance_to_obstacles_;
      arc_helpers::DoNotOptimize(in_collision);
    ...
    // If the grippers enter collision with the environment, then return however far we were able to get
    {
      stopwatch(RESET);
      arc_helpers::DoNotOptimize(next_grippers_poses);
      const bool in_collision = (sdf_->EstimateDistance3d(next_grippers_poses.first.translation()).first <
                                 gripper_min_distance_to_obstacles_) ||
                                (sdf_->EstimateDistance3d(next_grippers_poses.second.translation()).first <
                                 gripper_min_distance_to_obstacles_) ||
                                (sdf_->DistanceToBoundary3d(next_grippers_poses.first.translation()).first <
...
ROS_INFO_COND_NAMED(SMMAP_RRT_VERBOSE&& split_happened, "rrt.prop",
                    "Split happend during forward propagation, loop should terminate");
...
for (const std::pair<RubberBand::Ptr, double>& child_band : next_bands) {
  ...
  const RRTNode next_node(next_grippers_poses, next_robot_config, next_band, next_cost_to_come,
                          prev_node.pReachability() * p_transition, p_transition, parent_idx, next_state_index_,
                          next_transition_index_, split_happened ? next_split_index_ : -1);
...
ROS_INFO_COND_NAMED(SMMAP_RRT_VERBOSE&& split_happened, "rrt.prop",
                    "Split happend during forward propagation, this is immediately after the loop exit");
...
if (visualize) {
  const auto starting_idx =
      (&tree_to_extend == &forward_tree_) ? forward_tree_next_visualized_node_ : backward_tree_next_visualized_node_;

  const auto& tree_a_color =
      (&tree_to_extend == &forward_tree_) ? gripper_a_forward_tree_color_ : gripper_a_backward_tree_color_;

  const auto& tree_b_color =
      (&tree_to_extend == &forward_tree_) ? gripper_b_forward_tree_color_ : gripper_b_backward_tree_color_;

  const auto& tree_a_ns =
      (&tree_to_extend == &forward_tree_) ? RRT_FORWARD_TREE_GRIPPER_A_NS : RRT_BACKWARD_TREE_GRIPPER_A_NS;

  const auto& tree_b_ns =
      (&tree_to_extend == &forward_tree_) ? RRT_FORWARD_TREE_GRIPPER_B_NS : RRT_BACKWARD_TREE_GRIPPER_B_NS;

  const bool draw_band = true;
  visualizeTree(tree_to_extend, starting_idx, tree_a_ns, tree_b_ns, RRT_TREE_BAND_NS, tree_marker_id_, tree_marker_id_,
                1, tree_a_color, tree_b_color, band_tree_color_, draw_band);
  ++tree_marker_id_;
...
std::vector<std::pair<RubberBand::Ptr, double>> BandRRT::forwardPropogateBand(
    const RubberBand::ConstPtr& starting_band, const RRTGrippersRepresentation& next_grippers_poses,
  ...
  const auto transitions =
      transition_estimator_->estimateTransitions(*starting_band, test_gripper_positions, allow_mistakes);
...
void BandRRT::checkNewStatesForGoal(const ssize_t num_nodes) {
  ...
  for (size_t idx = forward_tree_.size() - num_nodes; idx < forward_tree_.size(); ++idx) {
    if (goalReached(forward_tree_[idx])) {
      ...
      if (forward_tree_[idx].pReachability() < 1.0) {
        ROS_INFO_STREAM_NAMED("rrt", "Goal reached idx["
                                         << idx << "]"
                                         << ", pReachability:         " << forward_tree_[idx].pReachability());
        ROS_INFO_STREAM_NAMED(
            "rrt", "                       , pGoalReachable[root]:  " << forward_tree_[0].getpGoalReachable());
        visualizeBothTrees();
        visualizeBlacklist();
      }
      //            PressKeyToContinue();
  ...
  if (force_nn_rebuild) {
    ...
    //        {
    //            ROS_INFO_STREAM_NAMED("rrt", "Sample history buffer size: " << sample_history_buffer_.size() << ".
    //            Saving to file."); ZlibHelpers::CompressAndWriteToFile(sample_history_buffer_, GetLogFolder(*nh_) +
    //            "/sample_history_buffer.compressed");
    //        }
    //        else
    //        {
    //            ROS_ERROR_STREAM_NAMED("rrt", "Sample history buffer size: " << sample_history_buffer_.size() << ".
    //            Unable to save to file.");
    //        }
    rebuildNNIndex(forward_nn_index_, forward_nn_raw_data_, forward_nn_data_idx_to_tree_idx_, forward_tree_,
                   forward_next_idx_to_add_to_nn_dataset_, force_nn_rebuild);
  }
}
...
bool BandRRT::goalReached(const RRTNode& node) {
  ...
  if (planning_for_whole_robot_) {
    const double gripper_a_rotation_dist =
        EigenHelpers::Distance(starting_grippers_poses_.first.rotation(), node.grippers().first.rotation());
    const double gripper_b_rotation_dist =
        EigenHelpers::Distance(starting_grippers_poses_.second.rotation(), node.grippers().second.rotation());
    if (gripper_a_rotation_dist > max_gripper_rotation_ || gripper_b_rotation_dist > max_gripper_rotation_) {
      return false;
    }

    if (visualization_enabled_globally_) {
      vis_->visualizeLineStrip(RRT_GOAL_TESTING_NS, node.band()->getVectorRepresentation(), Visualizer::White(), 1,
                               0.01);
    }
  }
...
void BandRRT::goalReachedCallback(const int64_t node_idx) {
  ...
  //        }
  //        arc_utilities::CreateDirectory(log_folder.GetImmutable());
  //        const auto file_name_prefix = ROSHelpers::GetParamRequiredDebugLog<std::string>(*ph_,
  //        "path_file_name_prefix", __func__); if (!file_name_prefix.Valid())
  //        {
  //            throw_arc_exception(std::invalid_argument, "Unable to load path_file_name_prefix from parameter
  //            server");
  //        }

  //        const std::string file_name_suffix = arc_helpers::GetCurrentTimeAsString();
  //        const std::string file_name = file_name_prefix.GetImmutable() + "__before_blacklisting_root_idx_" +
  //        std::to_string(root_index) + "__" + file_name_suffix + ".compressed"; std::string file_path =
  //        log_folder.GetImmutable() + file_name; storeTree(forward_tree_, file_path);
  //    }
  ...
  ROS_INFO_STREAM_COND_NAMED(SMMAP_RRT_VERBOSE, "rrt",
                             "Node " << node_idx << " reached goal, updating pGoalReachable from this node to root.");
  updatePGoalReachable(node_idx);

  ROS_INFO_STREAM_COND_NAMED(SMMAP_RRT_VERBOSE, "rrt",
                             "Root p_goal_reachable: " << forward_tree_[0].getpGoalReachable());
}
...
void BandRRT::updatePGoalReachable(const int64_t node_idx) {
  ...
  if (new_goal.getpGoalReachable() != 1.0) {
    throw_arc_exception(std::runtime_error,
                        "new_goal should have p_goal_reachable == 1.0 as it itself passes the goal check");
  }
  ...
  while (current_idx >= 0) {
    ...
    for (auto itr = effective_child_branches.begin(); itr != effective_child_branches.end(); ++itr) {
      ...
      ROS_INFO_STREAM_COND_NAMED(SMMAP_RRT_VERBOSE, "rrt",
                                 "Transition pGoalReachable " << p_goal_reachable_current_transition);
    ...
    if ((p_goal_reachable >= 0.999) && (p_goal_reachable <= 1.001) && (p_goal_reachable != 1.0)) {
      ROS_WARN_STREAM_NAMED(
          "rrt", "Total P(goal reached) = " << p_goal_reachable << ". Probably a numerical error, rounding to 1.0");
      p_goal_reachable = 1.0;
    }
    current_node.setpGoalReachable(p_goal_reachable);
    ROS_INFO_STREAM_COND_NAMED(SMMAP_RRT_VERBOSE, "rrt",
                               "Setting  pGoalReachable for node " << current_idx << " to " << p_goal_reachable);
...
static VectorVector3d findFirstGripperWaypoints(const RRTTree& path, const size_t start_index, const size_t end_index) {
  assert(start_index < end_index);
  assert(end_index < path.size());
  ...
  for (size_t idx = start_index; idx < end_index - 1; ++idx) {
    ...
    // If the straight line distance between the start and the next gripper does not match the path distance, then the
    // current node is a kink
...
static VectorVector3d findSecondGripperWaypoints(const RRTTree& path, const size_t start_index,
...
static VectorVector3d createOtherGripperWaypoints(const VectorVector3d& given_gripper_waypoints,
                                                  const Vector3d& start_point, const Vector3d& end_point) {
...
void BandRRT::shortcutSmoothPolicy(RRTPolicy& policy, const bool visualization_enabled_locally) {
  Stopwatch function_wide_stopwatch;
  ...
  for (size_t segment_idx = 0; segment_idx < policy.size(); ++segment_idx) {
    ...
    shortcutSmoothPath(segment, maintain_goal_reach_invariant, visualization_enabled_locally, (int32_t)segment_idx + 1);
  }
  ...
  smoothing_statistics_["smoothing5_forward_propogation_band_sim_time            "] =
      total_band_forward_propogation_time_ - transition_estimator_->classifierTime();
  smoothing_statistics_["smoothing6_classifier                                   "] =
      transition_estimator_->classifierTime();
  smoothing_statistics_["smoothing7_forward_propogation_first_order_vis_time     "] =
      total_first_order_vis_propogation_time_;
  smoothing_statistics_["smoothing8_forward_propogation_everything_included_time "] =
      total_everything_included_forward_propogation_time_;
  smoothing_statistics_["smoothing9_total_time                                   "] = smoothing_time;

  smoothing_statistics_["smoothing_classifier_num0_band_wierdness                "] =
      (double)transition_estimator_->numBandWeirdness();
  smoothing_statistics_["smoothing_classifier_num1_band_safe                     "] =
      (double)transition_estimator_->numBandSafe();
  smoothing_statistics_["smoothing_classifier_num2_band_overstretch              "] =
      (double)transition_estimator_->numBandOverstretch();
  smoothing_statistics_["smoothing_classifier_num3_band_no_mistake               "] =
      (double)transition_estimator_->numNoMistakes();
  smoothing_statistics_["smoothing_classifier_num4_band_mistakes                 "] =
      (double)transition_estimator_->numMistakes();
  smoothing_statistics_["smoothing_classifier_num5_band_accepted_mistakes        "] =
      (double)transition_estimator_->numAcceptedMistakes();
}

void BandRRT::shortcutSmoothPath(RRTPath& path, const bool maintain_goal_reach_invariant,
                                 const bool visualization_enabled_locally, const int32_t visualization_idx) {
  if (path.size() < 2) {
    return;
  }
  ...
  while (path.size() > 2 && num_iterations < max_smoothing_iterations_ &&
    ...
    const int64_t max_delta = std::min((int64_t)path.size() - base_index - 1 + max_shortcut_index_distance_ / 10,
                                       max_shortcut_index_distance_);
    ...
    const int64_t second_index =
        arc_helpers::ClampValue(base_index + offset_delta, (int64_t)0, (int64_t)path.size() - 1);
    ...
    if (planning_for_whole_robot_) {
      ...
      const double minimum_distance =
          RRTDistance::Distance(smoothing_start_config.robotConfiguration(), smoothing_end_config.robotConfiguration());
      ...
      forwardPropogationFunction(smoothed_segment, start_idx, smoothing_end_config, allow_mistakes,
      ...
      if (!robotConfigurationsAreApproximatelyEqual(last_robot_configuration, target_robot_configuration)) {
        ...
        //                          << "   Robot configuration equal? " <<
        //                          robotConfigurationsAreApproximatelyEqual(last_robot_configuration,
      ...
      end_of_smoothing_to_goal_results =
          forwardSimulateGrippersPath(path, smoothing_end_index, *smoothed_segment.back().band());
    } else {
      ...
      if (smoothing_type == 1 || smoothing_type == 2) {
        // Check if the edge possibly can be smoothed
        const double minimum_distance =
            RRTDistance::Distance(smoothing_start_config.grippers(), smoothing_end_config.grippers());
        const double path_distance =
            RRTDistance::GrippersPathDistance(path, smoothing_start_index, smoothing_end_index);
        // Essentially this checks if there is a kink in the path
        if (IsApprox(path_distance, minimum_distance, 1e-6)) {
          ROS_INFO_COND_NAMED(SMMAP_RRT_VERBOSE, "rrt.smoothing",
                              "Skipping smoothing attempt as there is very little kink in sampled sub path");
          ++failed_iterations;
          continue;
        }
        ...
        if (visualization_enabled_locally) {
          ...
          vis_->visualizeCubes(RRT_SMOOTHING_GRIPPER_A_NS, gripper_a_cubes, Vector3d(0.01, 0.01, 0.01),
                               gripper_a_forward_tree_color_, 1);
          vis_->visualizeCubes(RRT_SMOOTHING_GRIPPER_B_NS, gripper_b_cubes, Vector3d(0.01, 0.01, 0.01),
                               gripper_b_forward_tree_color_, 2);
        }
        ...
        if (smoothing_type == 3) {
          ...
          target_waypoints_first_gripper = createOtherGripperWaypoints(target_waypoints_second_gripper,
                                                                       first_gripper_start_pos, first_gripper_end_pos);
        }
        // Smooth the second gripper
          ...
          target_waypoints_second_gripper = createOtherGripperWaypoints(
              target_waypoints_first_gripper, second_gripper_start_pos, second_gripper_end_pos);
        }
        ...
        if (visualization_enabled_locally) {
          ...
          gripper_a_cubes.insert(gripper_a_cubes.end(), target_waypoints_first_gripper.begin(),
                                 target_waypoints_first_gripper.end());
          gripper_b_cubes.insert(gripper_b_cubes.end(), target_waypoints_second_gripper.begin(),
                                 target_waypoints_second_gripper.end());
        ...
        for (size_t waypoint_idx = 1; waypoint_idx < num_waypoints; ++waypoint_idx) {
          ...
          const RRTNode forward_prop_target_config(target_poses, target_config, path.front().band());

          const int64_t start_idx = (int64_t)smoothed_segment.size() - 1;
          forwardPropogationFunction(smoothed_segment, start_idx, forward_prop_target_config, allow_mistakes,
                                     fwd_prop_local_visualization_enabled);
          // Exit early if we hit any of "failure" conditions
          // Check if the we hit a split
          if (smoothed_segment.back().splitIndex() >= 0) {
            ROS_WARN_NAMED("rrt.smoothing",
                           "Split in path while smoothing single gripper, rejecting smoothing attempt");
            break;
          }
      ...
      if (!gripperPositionsAreApproximatelyEqual(last_gripper_position, target_gripper_position)) {
        ROS_INFO_COND_NAMED(SMMAP_RRT_VERBOSE, "rrt.smoothing",
                            "Shortcut failed, gripper positions don't match targets, thus the band got overstretched");
        ++failed_iterations;
        continue;
      }
    ...
    if (maintain_goal_reach_invariant) {
      ...
      if (!final_band_at_goal_success || final_band_visible_to_blacklist) {
        ROS_INFO_STREAM_COND_NAMED(SMMAP_RRT_VERBOSE, "rrt.smoothing",
                                   "Shortcut failed"
                                       << "    Band at goal? " << final_band_at_goal_success << "    Band visible? "
                                       << final_band_visible_to_blacklist);
        ++failed_iterations;
        continue;
      }
    }
    ...
    ///////////////////// Smoothing success - Create the new smoothed path /////////////////////////////////////////
    {
      ...
      smoothed_path.reserve((smoothing_start_index + 1) + (smoothed_segment.size() - 1) +
                            (end_of_smoothing_to_goal_path.size() - 1));
      ...
      // Insert the changed end of the path with the new rubber band - gripper/robot positions are identical (note that
      // we are removing the overlap as we go here)
      smoothed_path.insert(smoothed_path.end(), end_of_smoothing_to_goal_path.begin() + 1,
                           end_of_smoothing_to_goal_path.end());
 ...
 */
std::pair<bool, RRTTree> BandRRT::forwardSimulateGrippersPath(const RRTTree& path, const size_t start_index,
  ...
  // Put the start position on the path
  {
    ...
    resulting_path.push_back(RRTNode(path[start_index].grippers(), path[start_index].robotConfiguration(),
  ...
  while (!band_is_overstretched && !band_got_stuck && path_idx < path.size()) {
    ...
    rubber_band.forwardPropagate(ToGripperPositions(ending_grippers_poses), rubber_band_verbose);
    ...
    resulting_path.push_back(RRTNode(path[path_idx].grippers(), path[path_idx].robotConfiguration(),
                                     std::make_shared<RubberBand>(rubber_band)));
