 ...
 */
void ConstraintJacobianModel::SetInitialObjectConfiguration(const ObjectPointSet &object_initial_configuration) {
  if (!grippers_data_initialized_.load()) {
    throw_arc_exception(std::runtime_error,
                        "You must call DeformableModel::SetGrippersData before "
                        "setting the static data for ConstraintJacobianModel");
  }
  ...
  for (ssize_t gripper_ind = 0; gripper_ind < num_grippers; gripper_ind++) {
    for (ssize_t node_ind = 0; node_ind < num_nodes_; node_ind++) {
      const std::pair<double, long> nearest_node_on_gripper = GetMinimumDistanceIndexToGripper(
          grippers_data_[(size_t)gripper_ind].node_indices_, node_ind, object_initial_node_distance_);

      gripper_to_node_min_distances(gripper_ind, node_ind) = nearest_node_on_gripper.first;
    }
  }

  // object_node_to_grippers_distances_ is indexed first by gripper, second by
  // node i.e. (gripper_ind, node_ind)
...
ConstraintJacobianModel::ConstraintJacobianModel(std::shared_ptr<ros::NodeHandle> nh,
  ...
  if (!static_data_initialized_.load()) {
    throw_arc_exception(std::runtime_error,
                        "You must call SetInitialObjectConfiguration before "
                        "constructing a ConstraintJacobianModel");
  }
...
ObjectPointSet ConstraintJacobianModel::getObjectDelta_impl(
    const WorldState &world_state, const AllGrippersSinglePoseDelta &grippers_pose_delta) const {
...
/**
 * @brief
 * ConstraintJacobianModel::computeGrippersToDeformableObjectJacobian_impl
...
Eigen::MatrixXd ConstraintJacobianModel::computeGrippersToDeformableObjectJacobian(
  ...
  const kinematics::VectorIsometry3d grippers_next_poses =
  ...
  for (ssize_t gripper_ind = 0; gripper_ind < num_grippers; gripper_ind++) {
    ...
    // Due to the assumption of free-flying grippers, I simply take it as the
    // xyz motion of grippers In the future, it should be the translational
    // motion of end effector.
    const Vector3d &gripper_translation =
        grippers_next_poses[gripper_ind].translation() - grippers_current_poses[gripper_ind].translation();
    ...
    for (ssize_t node_ind = 0; node_ind < num_nodes_; node_ind++) {
      ...
      const Vector3d node_to_gripper =
          current_configuration.col(nearest_node_on_gripper_ind) - current_configuration.col(node_ind);
      ...
      const double direction_based_rigidity =
          dirPropotionalModel(node_to_gripper, gripper_translation, geodesic_distance_to_gripper);
      ...
      const double rigidity_translation =
          gripper_influence_per_node_(gripper_ind, node_ind) * distance_ratio_rigidity * direction_based_rigidity;
      ...
      const double rigidity_rotation = gripper_influence_per_node_(gripper_ind, node_ind) *
...
double ConstraintJacobianModel::dirPropotionalModel(const Eigen::Vector3d node_to_gripper,
