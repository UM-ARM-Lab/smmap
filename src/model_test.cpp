...
ModelTest::ModelTest(RobotInterface& robot,
    ...
    , test_planner_(robot_, vis_, test_specification_, logging_fn_, test_logging_fn_)
{
    initializeLogging();
}
...
//        test_logging_fn_(current_world_state,real_delta_p,model_delta_p);
        ...
        test_planner_.detectFutureConstraintViolations(current_world_state);
        ...
        world_feedback = test_planner_.sendNextCommand(current_world_state);

//        if (unlikely(world_feedback.sim_time_ - start_time >= test_specification_->maxTime()));
        if (world_feedback.sim_time_ - start_time < 0)
        {
            ROS_INFO_STREAM_NAMED("task", "Robot Shut dwom in Model_test " << world_feedback.sim_time_ << "\t ****: " << start_time);
            robot_.shutdown();
        }
    }
}
...
void ModelTest::initializeModelSet(const WorldState& initial_world_state)
{
             ...
             ph_.getParam("rotational_deformability", rotational_deformability))
    {
        ...
        test_planner_.addModel(std::make_shared<DiminishingRigidityModel>(
                              translational_deformability,
                              rotational_deformability,
    ...
    else if (GetUseMultiModel(ph_))
    {
        ...
        {
                test_planner_.addModel(std::make_shared<AdaptiveJacobianModel>(
...
//                              GetOptimizationEnabled(nh_)));
    ...
    else if (GetUseConstraintModel(ph_))
    {
        ...
        test_planner_.addModel(std::make_shared<ConstraintJacobianModel>(
...
void ModelTest::testLogData(const WorldState& current_world_state,
                            ...
                            const Eigen::MatrixXd& real_time_error,
                            const Eigen::MatrixXd& constraint_violation)
{
    if (logging_enabled_)
    {
        const Eigen::IOFormat single_line(
