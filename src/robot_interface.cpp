...
WorldState RobotInterface::start() {
  ...
  try {
    ...
    ROS_WARN("%s", ex.what());
    ROS_WARN(
        "Assuming this means that no transform has been broadcast from world to bullet, so assuming identity, but NOT "
        "broadcasting");
...
bool RobotInterface::ok() const { return ros::ok(); }
...
const std::vector<GripperData>& RobotInterface::getGrippersData() const { return grippers_data_; }
...
AllGrippersSinglePose RobotInterface::getGrippersPoses() {
  ...
  for (size_t gripper_ind = 0; gripper_ind < grippers_data_.size(); gripper_ind++) {
    ros::ServiceClient gripper_pose_client = nh_->serviceClient<GetGripperPose>(GetGripperPoseTopic(*nh_));
    ...
    if (!gripper_pose_client.call(pose_srv_data)) {
      ROS_FATAL_STREAM_NAMED("robot_interface",
                             "Unabled to retrieve gripper pose: " << grippers_data_[gripper_ind].name_);
    }
    CHECK_FRAME_NAME("robot_interface", world_frame_name_, pose_srv_data.response.header.frame_id);

    grippers_pose[gripper_ind] = GeometryPoseToEigenIsometry3d(pose_srv_data.response.pose);
  }
...
std::pair<WorldState, std::vector<WorldState>> RobotInterface::commandRobotMotion(
    const AllGrippersSinglePose& target_grippers_poses, const VectorXd& target_robot_configuration,
...
bool RobotInterface::testRobotMotion(const std::vector<AllGrippersSinglePose>& test_grippers_poses,
  ...
  return testRobotMotion_impl(
      toRosTestPosesGoal(test_grippers_poses, test_robot_configurations, robot_configuration_valid), feedback_callback);
}

bool RobotInterface::generateTransitionData(const std::vector<TransitionTest>& tests,
...
std::pair<WorldState, std::vector<WorldState>> RobotInterface::testRobotMotionMicrosteps(
    const VectorIsometry3d& starting_rope_configuration, const AllGrippersSinglePose& starting_grippers_poses,
    const AllGrippersSinglePose& target_grippers_poses, const int num_substeps) {
  return testRobotMotionMicrosteps_impl(toRosMicrostepsRequest(starting_rope_configuration, starting_grippers_poses,
                                                               target_grippers_poses, num_substeps));
}
...
const VectorXd& RobotInterface::getJointLowerLimits() const { return joint_lower_limits_; }

const VectorXd& RobotInterface::getJointUpperLimits() const { return joint_upper_limits_; }

const VectorXd& RobotInterface::getJointWeights() const { return joint_weights_; }
...
AllGrippersSinglePose RobotInterface::getGrippersPosesFunctionPointer() const {
  if (get_ee_poses_fn_ == nullptr) {
    ROS_ERROR_ONCE_NAMED("robot_interface",
                         "Asked for gripper poses (function pointer input), but function pointer is null");
...
VectorXd RobotInterface::mapGripperMotionToRobotMotion(const AllGrippersSinglePoseDelta& grippers_delta) const {
  ...
  //    std::cout << "Stacked delta size: " << stacked_gripper_delta.rows() << " x " << stacked_gripper_delta.cols() <<
  //    std::endl; std::cout << "Jacobian size     : " << jacobian.rows() << " x " << jacobian.cols() << std::endl;
...
std::vector<VectorXd> RobotInterface::getCloseIkSolutions(const AllGrippersSinglePose& target_poses,
...
std::vector<VectorXd> RobotInterface::getCloseIkSolutions(const std::vector<std::string>& gripper_names,
...
std::pair<bool, VectorXd> RobotInterface::getGeneralIkSolution(const VectorXd& starting_config,
...
void RobotInterface::setCallbackFunctions(
    ...
    const std::function<void()>& lock_env_fn, const std::function<void()>& unlock_env_fn,
    ...
    const std::function<std::vector<VectorXd>(const std::vector<std::string>& gripper_names,
                                              const AllGrippersSinglePose& target_poses,
                                              const double max_gripper_distance)>& close_ik_solutions_fn,
    const std::function<std::pair<bool, VectorXd>(const VectorXd& starting_config,
                                                  const std::vector<std::string>& gripper_names,
                                                  const AllGrippersSinglePose& target_poses)>& general_ik_solution_fn,
  ...
  assert(joint_info.size() == 3 &&
         "Joint info is assumed to be in the order [lower_limits, upper_limits, joint_weights]");
...
Vector3d RobotInterface::transformToFrame(const Vector3d& point, const std::string& source_frame,
                                          const std::string& target_frame, const ros::Time& time) const {
...
const Isometry3d& RobotInterface::getWorldToTaskFrameTf() const { return world_to_bullet_tf_; }
...
ExecuteRobotMotionRequest RobotInterface::toRosMovementRequest(const AllGrippersSinglePose& grippers_poses,
...
void RobotInterface::internalTestPoseFeedbackCallback(const TestRobotMotionActionFeedbackConstPtr& feedback,
                                                      const TestRobotMotionFeedbackCallback& feedback_callback) {
...
bool RobotInterface::testRobotMotion_impl(const TestRobotMotionGoal& goal,
                                          const TestRobotMotionFeedbackCallback& feedback_callback) {
  ...
  ros::Subscriber internal_feedback_sub = nh_->subscribe<TestRobotMotionActionFeedback>(
      GetTestRobotMotionTopic(*nh_) + "/feedback", 1000,
...
TestRobotMotionGoal RobotInterface::toRosTestPosesGoal(const std::vector<AllGrippersSinglePose>& grippers_poses,
                                                       const std::vector<VectorXd>& robot_configurations,
                                                       const bool robot_configurations_valid) const {
  assert(!robot_configurations_valid || robot_configurations.size() == grippers_poses.size());
  ...
  for (size_t pose_ind = 0; pose_ind < grippers_poses.size(); ++pose_ind) {
    goal.poses_to_test[pose_ind].poses = VectorIsometry3dToVectorGeometryPose(grippers_poses[pose_ind]);
  }
  ...
  for (size_t config_ind = 0; config_ind < robot_configurations.size(); ++config_ind) {
    goal.configurations_to_test[config_ind].configuration = EigenVectorXToStdVector(robot_configurations[config_ind]);
  }
...
bool RobotInterface::generateTransitionData_impl(const deformable_manipulation_msgs::GenerateTransitionDataGoal& goal,
                                                 const GenerateTransitionDataFeedbackCallback& feedback_callback,
                                                 const bool wait_for_feedback) {
  ...
  if (feedback_callback != nullptr) {
    internal_feedback_sub = nh_->subscribe<GenerateTransitionDataActionFeedback>(
        GetGenerateTransitionDataTopic(*nh_) + "/feedback", 1000,
...
TransitionTest RobotInterface::toRosTransitionTest(const EigenHelpers::VectorIsometry3d& starting_rope_node_transforms,
  ...
  test.starting_object_configuration = VectorIsometry3dToVectorGeometryPose(starting_rope_node_transforms);
  test.starting_gripper_poses = VectorIsometry3dToVectorGeometryPose(starting_gripper_poses);
  ...
  for (size_t path_idx = 0; path_idx < path_to_start_of_test.size(); ++path_idx) {
    test.path_to_start_of_test[path_idx].poses = VectorIsometry3dToVectorGeometryPose(path_to_start_of_test[path_idx]);
  ...
  test.final_num_substeps =
      (int)std::ceil(Distance(prev_poses, final_gripper_targets, false) / (max_gripper_velocity_norm_ * dt_));
...
std::pair<WorldState, std::vector<WorldState>> RobotInterface::testRobotMotionMicrosteps_impl(
  ...
  return {ConvertToEigenFeedback(result.start_after_settling), ConvertToEigenFeedback(result.microsteps)};
}

TestRobotMotionMicrostepsRequest RobotInterface::toRosMicrostepsRequest(
    const VectorIsometry3d& starting_rope_configuration, const AllGrippersSinglePose& starting_grippers_poses,
    const AllGrippersSinglePose& target_grippers_poses, const int num_substeps) const {
